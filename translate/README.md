...
- `translate/bin/translate-en [--dry-run] [--pipeline claude-code-series] [article.md ...]`：生成英文翻译计划、frontmatter 策略、链接/资产重写摘要和运行记录。真实翻译必须由 skill 展示摘要后再确认，并通过 GPT Codex (`codex exec`) 执行。
- `translate/bin/translate-ja [--dry-run] [--pipeline claude-code-series] [article.md ...]`：生成日语翻译计划、frontmatter 策略、链接/资产重写摘要和运行记录。真实翻译必须由 skill 展示摘要后再确认，并通过 GPT Codex (`codex exec`) 执行。
...
- `workflows/gpt-codex-cli.md`：正文真实翻译的 GPT Codex 调用边界和输出清理约束。
...
- 正文真实翻译只使用 GPT Codex；不得回退到 Claude Code、`claude` CLI 或旧 `scripts/translate-*` 入口。
