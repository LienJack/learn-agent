...
- `translate/bin/translate-assets [--dry-run] --language <en|ja> [--stage mermaid|images] [--only file]`：规划 Mermaid 渲染或普通 PNG/JPG 本地化；两个阶段严格分流。
- `translate/bin/translate-check --language <en|ja> [--pipeline claude-code-series] [article.md ...]`：检查目标 Markdown、frontmatter、链接、图片引用、残留污染和发布前置条件。
...
- `runs/`：每次运行的 `record.json`，记录输入、目标语言、产物、检查结果、blocker、warning 和人工确认点。

## 安全边界

- 翻译中控台只生成站内多语言 Markdown 和本地 assets；DEV、Zenn、微信公众号等发布动作仍由 `publish/` 负责。
- 真实模型翻译、Mermaid 渲染和图片生成前，skill 必须先运行 dry-run 或 check，展示摘要、产物路径、风险提示和人工确认点。
- Mermaid 图必须走独立流程：提取源 Mermaid、翻译可见标签、保留语法、渲染 PNG、写入目标 assets。
- 普通 PNG/JPG 图片不复用 Mermaid 渲染入口；V1 只保留 prompt 清单、参考图、尺寸/格式检查和可重跑线索。
