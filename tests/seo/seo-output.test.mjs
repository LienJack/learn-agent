...
test('built RAG article without a hero image uses stable fallback sharing image', async () => {
	...
	assertMeta(html, /"author":\{"@type":"Person","@id":"https:\/\/blog\.lienjack\.com\/about#lien-jack","name":"Lien Jack","alternateName":\["LienJack"\],"url":"https:\/\/blog\.lienjack\.com\/about","sameAs":\["https:\/\/github\.com\/LienJack"\]\}/, 'normalized author JSON-LD');
	assert.doesNotMatch(html, /undefined/);
});
...
test('built home and about pages expose personal identity and social sharing metadata', async () => {
	...
	for (const html of [home, about, enAbout, jaAbout]) {
	...
	assertMeta(about, /"@id":"https:\/\/blog\.lienjack\.com\/about#lien-jack"/, 'person id');
