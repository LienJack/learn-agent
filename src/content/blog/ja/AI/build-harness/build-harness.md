...
このシリーズは「問題 -> 仕組み -> 失敗パターン -> 次の仕組み」というリズムで読むと理解しやすくなります。各層を読むたびに、次の三つを確認してください。

```text
この仕組みはどんな痛みを解決するのか？
これがないと Agent はどこで制御を失う、または現実を取り違えるのか？
前後の層とどう接続されるのか？
```

まず全体像を掴みたい場合は 00-01 から 00-05 までを読みます。早く実装に入りたい場合は 00-07 のあと、00-08 から 00-16 で minimal loop、tool runtime、context policy、event log を組み立ててください。後半の capability discovery、delegation、trace、memory、retrieval、productization、hosting は、動く Agent を、出荷でき、診断でき、長く保守できる Harness へ育てる部分です。

## シリーズの道筋

### 第 1 部：Agent の基本モデルと Harness の進化
...
18. [Delegation Runtime：タスクを外へ渡しても、制御権を失わない](https://github.com/LienJack/build-harness/blob/main/docs/ja/00-18-delegation-runtime-control.md)  
    sub-agent とタスク委譲を導入しつつ、親 Harness が context、permission、結果の join、失敗復旧を握り続ける方法を扱います。

19. [Trace Analysis：事実ログで Agent 失敗を特定する](https://github.com/LienJack/build-harness/blob/main/docs/ja/00-19-trace-analysis-agent-failures.md)  
    失敗を感覚で推測するのではなく、trace から decision、observation、tool output、control point を辿って原因を特定します。
...
22. [Productized CLI：profile、extension、multi-provider](https://github.com/LienJack/build-harness/blob/main/docs/ja/00-22-productized-cli-profile-extension.md)  
    prototype を実際に使える CLI へ育てます。profile、provider selection、extension、安定した出力、予測可能な UX が焦点です。

23. [Hosted Harness：Sandbox、Cron、Durable Execution とリモートデプロイ](https://github.com/LienJack/build-harness/blob/main/docs/ja/00-23-hosted-harness-durable-execution.md)  
    ローカル実行から hosted execution へ進みます。sandbox、queue、retry、schedule、durable state によって production に耐える Agent にします。
