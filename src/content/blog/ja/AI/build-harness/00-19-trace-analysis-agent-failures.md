---
title: "Trace Analysis：事実ログで Agent 失敗を定位する"
...
# Trace Analysis：事実ログで Agent 失敗を定位する
...
父 Agent が外へ渡すのは局所作業であって、制御権ではない。
```

本篇が解くのは次の層の問題だ。

```text
事実ログがあるとして、それを失敗定位できる trace としてどう組織するか？
```
...
父 Agent は join 時にその unknown を無視したのか？
```
...
![Trace Analysis：事実ログで Agent 失敗を定位する Mermaid 1](assets/00-19-trace-analysis-agent-failures/mermaid-01.png)
...
![Trace Analysis：事実ログで Agent 失敗を定位する Mermaid 2](assets/00-19-trace-analysis-agent-failures/mermaid-02.png)
...
Agent 失敗を定位するには、trace は少なくとも八つの境界をつなげなければならない。
...
![Trace Analysis：事実ログで Agent 失敗を定位する Mermaid 3](assets/00-19-trace-analysis-agent-failures/mermaid-03.png)
...
![Trace Analysis：事実ログで Agent 失敗を定位する Mermaid 4](assets/00-19-trace-analysis-agent-failures/mermaid-04.png)
...
事実が sub-agent transcript にあることは、父 Agent join が継承したことを意味しない。
...
![Trace Analysis：事実ログで Agent 失敗を定位する Mermaid 5](assets/00-19-trace-analysis-agent-failures/mermaid-05.png)
...
子 Agent は旧 API risk を見つけたが、父 Agent には「問題なさそう」だけが返った。
...
## 十二、Delegation Join ミス：子 Agent が見つけたことを、父 Agent が正しく使うとは限らない
...
```text
子 Agent の発見は、父 Agent の最終決定にどう影響したか？
```

Multi-Agent タスクでは、失敗帰因は単一 Agent より複雑だ。

たとえば父 Agent がテスト修正のために二つの子タスクを派す。

```text
test-investigator：失敗テストを再現・定位する。
...
父 Agent が join 時に何を採用したか。
父 Agent が何を無視したか。
最終決定がどの evidence を引用したか。
```

![Trace Analysis：事実ログで Agent 失敗を定位する Mermaid 6](assets/00-19-trace-analysis-agent-failures/mermaid-06.png)

この図で重要なのは Join / Review だ。

Multi-Agent は投票ではない。

父 Agent は誰が自信満々かだけを見てはいけない。
...
join_ignored_evidence：父 Agent が返された証拠を無視した。
join_ignored_unknowns：父 Agent が未知を安全と扱った。
...
父 Agent の最終 messages だけを見れば、こうしかないかもしれない。
...
![Trace Analysis：事実ログで Agent 失敗を定位する Mermaid 7](assets/00-19-trace-analysis-agent-failures/mermaid-07.png)
...
父 Agent join が子タスク unknowns を無視した。
```
...
```text
Trace はログの装飾層ではなく、Harness の失敗帰因層である。
```
