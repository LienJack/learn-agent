...
tags:
  ...
  - 技術チュートリアル
aliases:
  - ChatBot Workflow Agent Harness の違い
  - Agent のシステム境界
  - いつ Agent が必要か
---
...
![システム境界：ChatBot、Workflow、Agent、Harness の違い Mermaid 1](assets/00-03-chatbot-workflow-agent-harness/mermaid-01.png)
...
![システム境界：ChatBot、Workflow、Agent、Harness の違い Mermaid 2](assets/00-03-chatbot-workflow-agent-harness/mermaid-02.png)
...
![システム境界：ChatBot、Workflow、Agent、Harness の違い Mermaid 3](assets/00-03-chatbot-workflow-agent-harness/mermaid-03.png)
...
![システム境界：ChatBot、Workflow、Agent、Harness の違い Mermaid 4](assets/00-03-chatbot-workflow-agent-harness/mermaid-04.png)
...
decision path + layered architecture。主図では「要求の不確実性からシステム形態を判断する」決定経路を表し、右側に小さな Harness の分層外殻を補う。

### 画面要素リスト

- 左側入口：ユーザー要求。アイコンは対話吹き出し。
- 第一分岐：対話だけが必要。ノードは ChatBot、アイコンはチャットウィンドウ。
- 第二分岐：フローが確定。ノードは Workflow、アイコンはフロー矢印。
- 第三分岐：動的判断。ノードは Agent、アイコンは循環矢印とレンチ。
- Agent が実環境へ入ると、右側に Harness の外部制御面が現れる。アイコンは外殻、盾、ログカード、検証チェック。
- ハイライト要素：Agent から Harness への矢印を淡い黄色で強調し、「複雑性コスト / 制御責任」を示す。
- 下部注釈：アップグレード経路ではなく、境界選択である。

### ポジティブ画像プロンプト

オフホワイトの紙背景、軽い紙の質感、黒い手描きサインペン線画、日本語の技術解説図。左側に「ユーザー要求」の入口があり、右方向へ ChatBot、Workflow、Agent の三つの主要判断経路へ分岐する。ChatBot はチャットウィンドウアイコン、Workflow は固定フロー矢印アイコン、Agent は循環矢印とレンチアイコンで表す。Agent ノードの右側に Harness の外部制御面を接続し、外殻、盾、ログカード、検証チェックで権限、状態、観測、復旧、検証を表す。Agent から Harness への矢印を淡い黄色でハイライトし、「複雑性コスト / 制御責任」とラベルする。全体はエンジニアリング白板スケッチ風で、線は少し不均一、ノードラベルは短く明瞭、余白は十分。

### ネガティブプロンプト

3D レンダリングにしない、サイバーパンクにしない、複雑なカラーグラデーションにしない、写真風にしない、密集した小文字を入れない、英語の長文を入れない、実在人物を入れない、会社ロゴを入れない、青紫のテック風背景にしない、四つの概念を低いものから高いものへの階段として描かない。
