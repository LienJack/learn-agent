...
![モデル、ループ、ツール、状態で Agent が構成され、外部 Harness が制御システムを提供することを示す](assets/00-01-agent-not-a-prompt/photo-00-agent-runtime-composition.jpg)
...
![Prompt、ChatBot、Agent、Harness の責任がどのように段階的に増えるかを説明する](assets/00-01-agent-not-a-prompt/photo-01-prompt-to-agent-evolution.jpg)
...
![Agent の基本定義：なぜ一文の Prompt ではないのか？ Mermaid 1](assets/00-01-agent-not-a-prompt/mermaid-01.png)
...
![単発回答と複数ターンの前進の違いを閉ループとして描き、状態と観測結果の回流を強調する](assets/00-01-agent-not-a-prompt/photo-02-agent-loop-reality-bridge.jpg)
...
![Agent の基本定義：なぜ一文の Prompt ではないのか？ Mermaid 2](assets/00-01-agent-not-a-prompt/mermaid-02.png)

この図の矢印方向に注意してください。モデルはツールを直接呼びませんし、状態を直接書きません。すべての外部アクションは Runtime を通ります。この分担はこの先も何度も出てきます。

同じ鎖を状態遷移図として描くこともできます。

![Agent の基本定義：なぜ一文の Prompt ではないのか？ Mermaid 3](assets/00-01-agent-not-a-prompt/mermaid-03.png)
...
![モデルの提案は権限、サンドボックス、ログ、検証を通ってから現実世界へ入る必要があることを示す](assets/00-01-agent-not-a-prompt/photo-03-control-system-catches-action.jpg)
...
![Agent の基本定義：なぜ一文の Prompt ではないのか？ Mermaid 4](assets/00-01-agent-not-a-prompt/mermaid-04.png)
