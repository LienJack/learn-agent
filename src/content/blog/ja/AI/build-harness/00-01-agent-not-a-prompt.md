...
![モデル、ループ、ツール、状態で Agent が構成され、外部 Harness が制御システムを提供することを示す](assets/00-01-agent-not-a-prompt/photo-00-agent-runtime-composition.png)
...
![Prompt、ChatBot、Agent、Harness の責任がどのように段階的に増えるかを説明する](assets/00-01-agent-not-a-prompt/photo-01-prompt-to-agent-evolution.png)
...
![単発回答と複数ターンの前進の違いを閉ループとして描き、状態と観測結果の回流を強調する](assets/00-01-agent-not-a-prompt/photo-02-agent-loop-reality-bridge.png)
...
![モデルの提案は権限、サンドボックス、ログ、検証を通ってから現実世界へ入る必要があることを示す](assets/00-01-agent-not-a-prompt/photo-03-control-system-catches-action.png)
