...
本章后文有 36 道综合题。这里先给出高频系统设计和排障题入口：

1. Channel、Mutex、RWMutex、Atomic、Context 在高并发架构中如何选型？
2. 如何用 Little's Law 推导并发数、队列长度和可接受排队时间？
3. Worker Pool、Semaphore、Rate Limiter、Admission Control 分别限制什么？
4. 依赖变慢十倍时，系统应该排队、拒绝、降级还是重试？
5. 为什么“增加 Goroutine 数量”不等于扩容？
6. 如何设计 deadline budget，避免每层都给自己完整 timeout？
7. 重试为什么必须有幂等、退避、Jitter 和 retry budget？
8. Singleflight 解决什么问题？Leader Context 取消后 follower 如何处理？
9. Graceful Shutdown 的顺序应该是什么？如何证明没有 Goroutine 泄漏？
10. P99 突然升高时，如何区分 CPU、GC、锁、Channel、网络和下游瓶颈？
11. Block Profile、Mutex Profile、Execution Trace、Flight Recorder 分别看什么？
12. 设计一个高并发下游聚合服务，如何给出容量边界、故障边界、指标和验收标准？
