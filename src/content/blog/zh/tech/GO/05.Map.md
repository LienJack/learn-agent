...
| `fatal error: concurrent map writes`、锁保护、`sync.Map` | Map 视角下会给方案矩阵；happens-before 看 [第 11 章：并发基础](/blog/tech/GO/11.并发基础-Goroutine生命周期与Go内存模型)，Mutex/RWMutex 与 `sync.Map` 看 [第 13 章：sync 工具箱](/blog/tech/GO/13.Mutex-RWMutex与sync工具箱)，Atomic 看 [第 15 章：Atomic、CAS](/blog/tech/GO/15.Atomic-CAS-内存语义与无锁思想)。 |
...
![第 5 章：Map flow 1](./assets/05-map/mermaid-01.png)
...
![一图读懂 Go Map](./assets/05-map/go-map-infographic.webp)
...
所以 Swiss Table 快的核心不是“不比较 Key”，而是**先用很便宜的小标签过滤，减少昂贵的完整 Key 比较**；同时 group 连续存放，扫描 control bytes 对 CPU cache 更友好。

![Swiss Table 查找原理：先看小标签，再比较真实 Key](./assets/05-map/swiss-table-lookup.webp)
