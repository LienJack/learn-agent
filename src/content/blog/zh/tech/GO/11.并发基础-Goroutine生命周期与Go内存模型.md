...
本章后文保留完整回答与追问，这里先把高频题目集中列出来：

1. 并发和并行有什么区别？`GOMAXPROCS` 调大后吞吐一定提升吗？
2. Goroutine 与 OS Thread 有什么不同？为什么轻量不等于可以无限创建？
3. 仅仅 `go f()` 为什么不能保证调用者看到 `f` 的写入？
4. Sequenced-Before、Synchronized-Before、Happens-Before 和 DRF-SC 分别是什么？
5. Data Race 和 Race Condition 有什么区别？`go test -race` 没报警说明什么？
6. `WaitGroup.Go` 与 `Add/Done/Wait` 的差异是什么？为什么 `Add` 通常要先于 `go`？
7. Context 取消到底保证什么？为什么 cancel 后还要等待 Goroutine 退出？
8. GMP 中 G、M、P 分别保存什么？全局队列、本地队列和 Work Stealing 如何协作？
9. 下面代码用 `time.Sleep` 等 Goroutine 完成，为什么仍然错？
10. 服务关闭时如何设计“停止接流量、取消剩余任务、等待已接任务、释放资源”的顺序？
11. Goroutine profile 中大量 `<-ctx.Done()`、`chan send` 或 `IO wait` 分别可能说明什么？
12. 设计一个可取消、有限并发、结果顺序稳定的任务执行器，需要哪些不变量？
