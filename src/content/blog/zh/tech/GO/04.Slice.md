...
| 并发读写同一个 Slice、并发 `append`、data race | 本章只讲容器语义；happens-before 看 [第 11 章：并发基础](/blog/tech/GO/11.并发基础-Goroutine生命周期与Go内存模型)，锁和 Atomic 分别看 [第 13 章：sync 工具箱](/blog/tech/GO/13.Mutex-RWMutex与sync工具箱) 与 [第 15 章：Atomic、CAS](/blog/tech/GO/15.Atomic-CAS-内存语义与无锁思想)。 |
