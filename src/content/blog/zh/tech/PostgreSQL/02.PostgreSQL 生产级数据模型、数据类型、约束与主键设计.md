...
| Snowflake ID | Distributed ID | 应用层生成的 64 位趋势递增整数，通常由时间戳、机器 ID 和序列号组成 | 数据库 Sequence、严格提交顺序 | 标识/分布式 |
...
![PostgreSQL 生产级数据模型、数据类型、约束与主键设计 flow 1](./assets/02-postgresql/mermaid-01.png)
...
#### `bigint`、Snowflake、UUID v4、UUID v7

| 方案 | 主要优点 | 主要代价 | 典型用途 |
|---|---|---|---|
| `bigint Identity` | 8 字节、比较快、索引紧凑、插入局部性高 | 依赖数据库分配；可枚举；右侧热点可能竞争 | 单主写入的内部主键 |
| Snowflake BIGINT | 8 字节；应用层生成；多机器全局唯一；趋势递增；可贯穿 DB、Redis、MQ、日志和回调 | 依赖时钟、机器 ID 分配和序列耗尽处理；暴露粗时间；不是数据库提交序 | 分布式任务 ID、跨服务链路 ID、多写入口但仍希望保持紧凑索引 |
...
#### 分布式场景：Snowflake BIGINT

Snowflake ID 是应用层生成的 64 位整数，在 PostgreSQL 中通常存为 `bigint`。经典结构可以理解为：

```text
| 1 bit 符号位 | 41 bit 时间戳 | 10 bit 机器 ID | 12 bit 序列号 |
```

时间戳让 ID 大体随时间递增，机器 ID 区分不同生成节点，序列号解决同一机器同一毫秒内的多次生成。经典 12 bit 序列号意味着单机器单毫秒最多 4096 个 ID；超过上限时生成器必须等待下一毫秒或采取明确的降级策略。

它适合这样的链路：请求进入服务后先生成 `task_id`，再把同一个 ID 写入 PostgreSQL 主表、Redis 状态键、消息队列 Key、日志 Trace 字段和第三方回调参数。这样即使数据库写入、消息投递、前端轮询和回调发生在不同系统中，仍可用同一个稳定 ID 串起全链路。

```sql
CREATE TABLE app.video_task (
    ...
    CONSTRAINT uq_video_task_request UNIQUE (tenant_id, request_id)
);

CREATE INDEX ix_video_task_tenant_created
    ON app.video_task (tenant_id, created_at DESC, task_id);
```

应用写入时显式传入 ID：

```sql
INSERT INTO app.video_task (task_id, tenant_id, user_id, prompt, status, request_id)
VALUES ($1, $2, $3, $4, 'pending', $5);
```

相对 `bigint Identity`，Snowflake 不需要先访问数据库就能拿到 ID，适合多服务、多机器、多个写入入口共享同一任务编号。相对 UUID v4/v7，它仍是 8 字节整数，主键索引、外键和消息载荷更紧凑。相对数据库 Sequence，它把正确性压力从 PostgreSQL 转移到了 ID 生成器：必须保证 `worker_id` 不重复，检测时钟回拨，处理同毫秒序列耗尽，并在扩容、容器重启、主备切换和多机房部署时避免机器 ID 复用。

Snowflake 的“趋势递增”也不等于严格全局单调，更不等于事务提交顺序。跨机器时钟偏差、批量生成、排队和事务等待都可能让较大的 ID 先提交或较小的 ID 后提交。若业务需要审计顺序、账务流水号或无空洞编号，仍应使用独立的业务排序/审计机制，而不是把 Snowflake 主键当作最终顺序。
...
| 生成列 | 由本行基础列计算出的派生列 | 只能用不可变表达式；不能直接写入；不等于 Identity |

生成列是一种**由数据库根据同一行其他字段自动计算出来的列**。业务代码不应该手动写入它；当依赖字段插入或更新后，PostgreSQL 会按定义的表达式重新得到生成列的值。它适合表达“永远可以由基础字段稳定推导出来”的结果，例如订单明细小计、规格化邮箱、规格化 SKU、JSONB 中常用字段提取等。
...
INSERT INTO app.order_item (quantity, unit_price)
VALUES (3, 19.90);
...
这里 `total_price` 不是插入参数，而是由 `quantity * unit_price` 计算得到。之后如果更新 `quantity` 或 `unit_price`，`total_price` 也会跟着重新计算，避免应用层漏改造成“数量、单价和小计不一致”。

生成列容易和两个机制混淆：

| 机制 | 例子 | 核心区别 |
|---|---|---|
| `DEFAULT` | `created_at timestamptz DEFAULT now()` | 只在 INSERT 省略该列时给默认值；通常不会因其他列变化而自动更新 |
| Identity | `id bigint GENERATED ALWAYS AS IDENTITY` | 由 Sequence 生成标识符，常用于主键；不是计算字段 |
| 生成列 | `total_price GENERATED ALWAYS AS (...) STORED` | 根据同一行基础字段计算，不能被业务手动覆盖 |

使用生成列时要先问三个问题：

1. 这个值是否能完全由同一行其他字段推导出来？
2. 表达式是否稳定、确定，不依赖当前时间、随机数、外部服务或其他表？
3. 这个计算结果是否需要被查询、排序、唯一约束或索引反复使用？

适合的例子包括 `lower(email)`、`lower(sku)`、`quantity * unit_price`、`payload ->> 'type'`。不适合的例子包括订单最终应付金额、人工调整价、账务流水号、依赖其他表汇总的余额、依赖 `now()` 或随机数的字段。后者有独立业务含义或外部状态，应该用普通列、明细表、触发器、物化汇总或应用事务逻辑明确维护。
...
因此 8 字节 `bigint` 与 16 字节 UUID 的差异不是只存在于主键索引。Snowflake 在 PostgreSQL 里仍是 8 字节 `bigint`，宽度接近 Identity，但生成位置从数据库移到应用层。复合 `(tenant_id uuid, entity_id uuid)` 还会进一步放大。UUID v7 改善的是插入时间局部性，不会把 16 字节变成 8 字节。

### 6.3 插入局部性、Page Split 与热点

单调 `bigint`、Snowflake BIGINT 或时间有序 UUID 通常向 B-tree 右侧叶页插入，最近页面容易留在缓存，随机读写较少；当并发极高时，同一批右侧页面也可能成为锁与缓存争用点。UUID v4 把写入分散到整个树，降低单一右侧热点，却增加页面随机触达、缓存淘汰和中间页分裂机会。
...
| 分布式任务 ID | 应用层 Snowflake BIGINT + 数据库 PK/UNIQUE | 各服务各用本地自增 ID | 写库前拿到全局 ID，贯穿 Redis/MQ/日志/回调 | 8 字节索引紧凑，仍有右侧页压力 | 生成器无 DB 往返，但需处理同毫秒序列上限 | worker_id、时钟回拨、扩容和多机房分配需运维治理 |
...
- **索引维护**：每个 INSERT 都要维护所有相关索引；每个 UPDATE 是否更新索引取决于被修改列和物理条件。主键越宽，唯一检查和缓存成本越高。Snowflake 虽是 8 字节，但趋势递增写入仍可能集中在右侧叶页。
...
- **热点索引页**：单调 ID 或 Snowflake 的高并发插入集中于右侧页；随机 UUID 扩大缓存触达，两者是不同代价。
...
- **应用层 ID 生成器**：Snowflake 生成器不依赖主库 Sequence，但 HA 责任转移到了应用和配置中心。主备切换、容器重启、自动扩容和跨机房部署都必须防止 `worker_id` 复用，并把时钟回拨作为可观测故障处理。
- **脑裂与 Fencing**：即使主键是 UUID 或 Snowflake，也不能让两个主库无协调写入后“自然合并”；唯一键、库存和业务状态都会冲突。Fencing 是集群层责任。
...
| 9 | Snowflake worker_id 随机分配或可复用 | 重启/扩容/跨机房后可能重复 ID | 配置中心分配、时钟回拨保护、DB 唯一兜底 |
| 10 | 把 Sequence 当无空洞单据号 | 回滚/缓存/崩溃产生空洞 | 独立审计编号流程 |
| 11 | 只在 Go 中检查唯一或库存 | 并发竞态 | 数据库约束/原子 DML |
| 12 | 建 FK 不建引用侧索引 | 父表删除更新扫描子表 | 建匹配访问模式的前缀索引 |
| 13 | 多租户唯一键/FK 不含 tenant_id | 跨租户冲突或引用泄漏 | 复合键与全链路 tenant 条件 |
| 14 | 热点表长期 `SELECT *` | TOAST、网络、Go 分配放大 | 显式投影、垂直拆分 |
| 15 | `updated_at DEFAULT now()` 后不更新 | 审计字段失真 | UPDATE/触发器显式维护 |
| 16 | CHECK 调用语义会变化的函数 | 旧行与新规则不一致 | 不可变表达式，变更时重建/校验 |
| 17 | 无测量地反规范化和复制字段 | 漂移、写放大、恢复困难 | 权威源、陈旧 SLO、重建 Runbook |
| 18 | Collation 升级后只 `REFRESH VERSION` | 索引仍按旧规则排列 | 先检测冲突并重建依赖对象 |
| 19 | 用新请求 ID重试提交未知的扣款/库存 | 重复业务动作 | 稳定幂等键 + 查询确认 |
...
### 18.7【原理/排障】bigint Identity、Snowflake、UUID v4、UUID v7 如何权衡？
...
6. **题目**：分布式任务系统为什么可能选择 Snowflake BIGINT，而不是数据库 Identity 或 UUID v4？
   **答案**：Snowflake 可在写库前由应用生成，能让同一个 `task_id` 贯穿 PostgreSQL、Redis、MQ、日志和回调；它仍是 8 字节整数，索引和载荷比 UUID 紧凑。代价是必须治理 worker_id、时钟回拨和同毫秒序列耗尽，并用数据库 PK/UNIQUE 做最终兜底。
