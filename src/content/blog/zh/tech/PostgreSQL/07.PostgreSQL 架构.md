...
![PostgreSQL 进程架构模型](./assets/07-process-architecture.jpg)

上图展示了 PostgreSQL 的经典进程架构。Postmaster 是所有进程的"老祖宗"，负责监听端口、接受连接、管理子进程。

![PostgreSQL Postmaster 与 Backend 多进程模型](./assets/07-photo-01-process-model.jpg)
...
![PostgreSQL 内存架构](./assets/07-memory-architecture.jpg)

上图展示了 PostgreSQL 的内存架构——共享内存是所有进程的数据交换中心，每个进程还有自己的私有内存。

![PostgreSQL 共享内存与 Backend 私有内存分工](./assets/07-photo-02-memory-model.jpg)
...
![PostgreSQL 多进程模型的优缺点和适用场景](./assets/07-process-tradeoffs.jpg)
...
![PgBouncer 将大量应用连接收敛成少量 PostgreSQL Backend](./assets/07-photo-03-connection-pool.jpg)
