...
```mermaid
flowchart LR
    P[Producer] --> CL[CommitLog<br/>全局追加日志]
    CL -->|异步分发| CQ0[TopicA/queue-0<br/>ConsumeQueue]
    CL -->|异步分发| CQ1[TopicA/queue-1<br/>ConsumeQueue]
    CL -->|异步分发| CQ2[TopicB/queue-0<br/>ConsumeQueue]
    CL -->|异步建索引| IDX[IndexFile<br/>Topic#Key → 物理偏移候选]
    CQ0 -->|phyOffset + size| CL
    CQ1 -->|phyOffset + size| CL
    CQ2 -->|phyOffset + size| CL
    IDX -->|phyOffset| CL
```
...
sequenceDiagram
    ...
    participant S as SendMessageProcessor〔R5.5〕
    participant D as DefaultMessageStore〔R5.5〕
    participant L as CommitLog〔R5.5〕
    participant M as DefaultMappedFile〔R5.5〕
    participant F as FlushManager〔R5.5〕
    participant R as ReputMessageService〔R5.5〕
    participant Q as ConsumeQueue/IndexFile

    C->>S: 发送消息
    S->>D: asyncPutMessage
    D->>L: asyncPutMessage
    L->>L: 分配 QueueOffset、编码、加写锁
    L->>M: appendMessage
    M-->>L: wroteOffset + wroteBytes
    L->>F: handleDiskFlush
    F-->>C: 根据刷盘/复制条件完成结果
    R->>L: 从 reputFromOffset 顺序解析
    R->>Q: doDispatch 构建 CQ 与 Index
    R-->>R: 推进 reputFromOffset
```
...
`MappedFile〔R5.5〕` 在 5.5.0 中是接口，默认实现为 `DefaultMappedFile〔R5.5〕`。`MappedFileQueue〔R5.5〕` 将多个固定大小文件组织为一条连续逻辑空间。新文件由 `AllocateMappedFileService#putRequestAndReturnMappedFile〔R5.5〕` 提交给后台线程预创建，通常同时准备“下一个”和“下下个”文件，减少切文件时的创建、扩容和 mmap 延迟；开启 warmMappedFile 后还可预触页并锁页，但会消耗启动时间和内存。

三个位置必须分清：

- `wrotePosition`：应用已写到哪里。
- `committedPosition`：使用 TransientStorePool 时，堆外临时缓冲已提交到 FileChannel/Page Cache 的位置。
- `flushedPosition`：已通过 `force` 推进的刷盘位置。

默认 `writeWithoutMmap=false` 时，写入落到可写 `MappedByteBuffer`；读取通过映射切片。5.5.0 还提供 `writeWithoutMmap=true`：写侧使用 FileChannel，但仍建立只读 mmap 供读取，且与 TransientStorePool 互斥。这一选项本身就说明 mmap 不是不可替代的性能真理。

### 7.7.1 mmap 是否一定比普通 IO 快

**不一定。** mmap 的优势是省去显式 `read()` 到用户缓冲区的步骤，可按地址访问并复用 Page Cache；但首次访问仍可能触发缺页，随机访问会增加 major fault、TLB 压力和回收开销。工作集大于内存时，映射范围再大也不会凭空获得缓存；反而可能因预读和淘汰造成抖动。普通 FileChannel 配合直接缓冲、批量读写，在访问模式可控时同样可能更稳定。

判断标准不是“用了 mmap 就快”，而是：访问是否局部、工作集能否驻留、缺页成本是否可接受、刷盘策略是否匹配、内存回收是否健康，以及故障时是否能正确处理映射文件生命周期。

### 7.7.2 “零拷贝”应如何准确表述

- **mmap**：把文件页映射到进程地址空间，避免应用先 `read` 到一个额外的用户态数组；数据仍需从存储设备进入内存，访问仍会经历页表、缺页和缓存一致性处理。
- **sendfile**：典型语义是让内核在文件与 socket 之间搬运，避免“内核文件缓存 → 用户缓冲 → 内核 socket 缓冲”的往返；仍可能存在设备到内存、内核到网卡路径上的 DMA 或复制。
- **RocketMQ 5.5.0 的经典拉取路径**：`ManyMessageTransfer#transferTo〔R5.5〕` 实现 Netty `FileRegion`，但源码是把 Header 和映射得到的 ByteBuffer 写入 `WritableByteChannel`，并非直接调用 `FileChannel#transferTo`。所以不能笼统宣称“所有 RocketMQ 消费读取都使用 Linux sendfile 且完全零复制”。更准确的说法是：它尽量避免不必要的用户态中间拷贝，具体复制次数取决于缓冲类型、Netty 传输实现、TLS、压缩和操作系统。


### 7.7.3 一次写入穿过哪些内存层次

默认 mmap 写路径中，`MessageExtEncoder#encode〔R5.5〕` 先在可复用的直接缓冲区中组织二进制记录，追加回调再把记录写到 CommitLog 对应的映射区域。映射页本质上与内核 Page Cache 中的文件页关联；修改映射区域后，页面成为脏页，随后由 `MappedByteBuffer#force` 或内核回写机制推进到存储设备。这里没有“绕过内核直接写盘”，也不是 Java 对象一变更就已经持久化。

开启 TransientStorePool 时，写入先落在预借用的堆外缓冲；`DefaultMappedFile#commit〔R5.5〕` 再把已写区间提交给 FileChannel，之后刷盘线程调用 force。于是 `wrotePosition > committedPosition > flushedPosition` 的窗口真实存在。该模式的价值在于隔离直接写 Page Cache 时可能受到的内存回收干扰，并把 commit 与 flush 分成两个可批量化阶段；代价是额外内存、一次缓冲到文件通道的搬运，以及更复杂的水位监控。

`writeWithoutMmap=true` 时，写侧经 FileChannel，读侧仍可使用只读映射。无论哪种模式，真正决定恢复边界的不是 Java 变量中的 position，而是启动后按文件内容重新验证出的最后完整记录位置。内存水位只是运行期进度指示器，不能替代磁盘格式中的长度、MagicCode 和 CRC。

### 7.7.4 预分配不等于预持久化

`AllocateMappedFileService〔R5.5〕` 提前创建并映射文件，是为了把文件创建、映射和可选预热的延迟移出消息切文件的关键路径。预分配后的 1 GiB 文件即使物理长度完整，也可能一条有效消息都没有；文件长度因此不能代表有效数据长度。预热也只是提前触页或锁页，不会把未来消息写入，更不会提升已经返回成功消息的持久化等级。
...
sequenceDiagram
    ...
    participant P as PullMessageProcessor〔R5.5〕
    participant D as DefaultMessageStore〔R5.5〕
    participant Q as ConsumeQueueInterface〔R5.5〕
    participant L as CommitLog〔R5.5〕
    participant T as ManyMessageTransfer〔R5.5〕

    C->>P: topic, queueId, queueOffset
    P->>D: getMessage
    D->>Q: iterateFrom(queueOffset)
    Q-->>D: CqUnit(phyOffset,size,tagsCode)
    D->>D: ConsumeQueue 级过滤
    D->>L: getMessage(phyOffset,size)
    L-->>D: MappedByteBuffer slice
    D->>D: CommitLog 级过滤并组装结果
    D-->>P: GetMessageResult
    P->>T: Header + message buffers
    T-->>C: 写入网络通道
```
