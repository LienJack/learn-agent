...
本章是性能优化、可观测性、故障诊断和综合面试的收束章节。这里会再次出现“Redis 为什么快”“Pipeline 是否越大越好”“连接池如何设置”“AOF 如何选择”“主从复制延迟如何排查”等题目，但它们是面试和排障入口，不替代前文主章节：高性能原理见[第 12 章](/blog/tech/Redis/12.Redis 高性能原理/)，Go 客户端见[第 2 章](/blog/tech/Redis/02.使用 Go 操作 Redis/)，持久化见[第 14 章](/blog/tech/Redis/14.RDB、AOF 与数据恢复/)，复制与高可用见[第 15 章](/blog/tech/Redis/15.主从复制与数据一致性/)到[第 17 章](/blog/tech/Redis/17.Redis Cluster 分片与高可用/)，缓存架构和并发控制见[第 18 章](/blog/tech/Redis/18.高并发缓存架构/)与[第 19 章](/blog/tech/Redis/19.分布式锁、限流、幂等与并发控制/)。
...
| 大 Key、热 Key、内存上涨  | 诊断路径和指标选择     | [第 3 章](/blog/tech/Redis/03.Key 设计、TTL 与生命周期管理/)、[第 13 章](/blog/tech/Redis/13.内存管理、过期删除与淘汰策略/) |
| AOF、RDB、fork、COW | 延迟尖峰和恢复风险入口   | [第 14 章](/blog/tech/Redis/14.RDB、AOF 与数据恢复/) |
| 复制延迟、故障转移、分片扩容 | 综合排障和容量规划入口   | [第 15 章](/blog/tech/Redis/15.主从复制与数据一致性/)、[第 16 章](/blog/tech/Redis/16.Sentinel 高可用/)、[第 17 章](/blog/tech/Redis/17.Redis Cluster 分片与高可用/) |
| 缓存雪崩、热 Key、回源保护 | 系统设计题入口       | [第 18 章](/blog/tech/Redis/18.高并发缓存架构/) |
| Lua、锁、限流、幂等、延迟任务 | 并发控制和一致性入口    | [第 19 章](/blog/tech/Redis/19.分布式锁、限流、幂等与并发控制/) |
