...
本章是 Key 命名、TTL API 和生命周期管理的主章节。Redis 如何在内存层执行过期删除、主动过期、淘汰和碎片治理，统一见[第 13 章](/blog/tech/Redis/13.内存管理、过期删除与淘汰策略/)；缓存雪崩、击穿、穿透和热 Key 的架构治理见[第 18 章](/blog/tech/Redis/18.高并发缓存架构/)；不要把 Keyspace Notification 当可靠延迟队列，List、Streams 与延迟任务的可靠性边界见[第 8 章](/blog/tech/Redis/08.PubSub、List 队列与 Streams/)和[第 19 章](/blog/tech/Redis/19.分布式锁、限流、幂等与并发控制/)。
