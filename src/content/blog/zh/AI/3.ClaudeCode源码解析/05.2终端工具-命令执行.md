---
title: 5.2 终端工具：Agent 真正碰到机器的那一刻
...
flowchart LR
    ...
    S --> R["读取工具<br/>Read"]
    R --> E["编辑工具<br/>Edit / Write"]
    E --> B["终端工具<br/>Bash / PowerShell"]
    B --> V["验证结果<br/>测试 / 构建 / git 状态"]
    V --> M["tool_result<br/>回到模型主循环"]
```
...
```mermaid
flowchart LR
    M["模型发起 Bash tool call"] --> P["权限系统<br/>allow / ask / deny"]
    P -->|deny| X["拒绝执行"]
    P -->|ask| U["用户确认"]
    U -->|拒绝| X
    U -->|允许| S["shouldUseSandbox"]
    P -->|allow| S
    S --> W["SandboxManager.wrapWithSandbox"]
    W --> E["spawn shell process"]
    E --> O["stdout / stderr / exit code"]
    O --> R["tool_result 回到主循环"]
```
