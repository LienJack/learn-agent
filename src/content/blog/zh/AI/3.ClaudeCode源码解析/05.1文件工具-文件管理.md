---
title: 5.1 文件工具：Agent 动手改代码前，先建立一份现场记录
...
# 5.1 文件工具：Agent 动手改代码前，先建立一份现场记录
...
```mermaid
flowchart LR
    U["用户任务<br/>修复登录失败"] --> S["搜索工具<br/>Glob / Grep"]
    S --> R["Read<br/>读取文件并记录基线"]
    R --> E["Edit<br/>基于 old_string 做局部替换"]
    R --> W["Write<br/>创建或完整重写"]
    E --> D["diff / history / LSP 通知"]
    W --> D
    D --> T["测试验证<br/>Bash"]
```
...
```mermaid
flowchart TB
    FILE["文件管理工具"] --> READ["Read<br/>观察文件并建立基线"]
    FILE --> EDIT["Edit<br/>局部、可审计修改"]
    FILE --> WRITE["Write<br/>创建或完整重写"]
    FILE --> NB["NotebookEdit<br/>Notebook 专用结构化编辑"]

    READ --> STATE["readFileState<br/>记录内容与 mtime"]
    EDIT --> STATE
    WRITE --> STATE

    EDIT --> DIFF["structuredPatch / diff"]
    WRITE --> DIFF
    EDIT --> LSP["LSP / VSCode 通知"]
    WRITE --> LSP
```
...
```mermaid
flowchart TB
    T["Tool Input"] --> P["规范化 path<br/>expandPath"]
    P --> R{"读还是写？"}
    R -->|Read / Glob / Grep| RP["read permission"]
    R -->|Edit / Write| WP["write permission"]
    WP --> S["额外写入校验<br/>secret / settings / mtime"]
    RP --> C["call"]
    S --> C
```
...
sequenceDiagram
    ...
    participant UI as UI/LSP

    M->>R: Read(auth.ts)
    R->>FS: read file
    R->>S: save content + mtime
    R-->>M: file content with line numbers

    M->>E: Edit(auth.ts, old_string, new_string)
    E->>S: check lastRead
    E->>FS: read current file + mtime
    E->>E: verify old_string unique
    E->>FS: write updated file
    E->>S: update content + mtime
    E->>UI: notify diff / LSP
    E-->>M: file updated
```
