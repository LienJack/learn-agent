---
title: 5.3 工作流工具：从 Todo 到 Task，Agent 如何把要做的事变成可追踪状态
...
```mermaid
flowchart TB
    User["用户目标"] --> Plan["Plan / Todo<br/>拆出工作步骤"]
    Plan --> ListTask["任务清单 Task<br/>TaskCreate / TaskUpdate / TaskList"]
    ListTask --> Agent["Agent / Teammate<br/>认领或执行任务"]
    Agent --> Runtime["运行时 Task<br/>local_bash / local_agent / remote_agent"]
    Runtime --> Output["TaskOutput<br/>读取输出"]
    Runtime --> Stop["TaskStop<br/>停止执行体"]
    Output --> ListTask
    Stop --> ListTask
```
