---
title: 8.2 业界的 Agent 协作：多 Agent 不是多开几个聊天框
...
sequenceDiagram
    ...
    U ->> S: 修复登录失败 bug
    S ->> E: fan-out: 查 auth 调用链
    S ->> T: fan-out: 找复现、跑测试
    S ->> R: fan-out: 审安全风险
    E -->> S: 返回调用链摘要
    T -->> S: 返回测试结果
    R -->> S: 返回风险 findings
    Note over S: fan-in: 综合三方结论
    S ->> S: 决策修复方案
    S -->> U: 最终结论与下一步
```
...
```mermaid
flowchart TD
    U["用户：修复登录失败 bug"] --> S["Supervisor / 主 Agent"]
    S --> T["Task Ledger"]
    S --> E["explorer<br/>查 auth 调用链"]
    S --> R["docs-researcher<br/>校准框架 API"]
    S --> Q["tester<br/>复现失败测试"]
    E --> T
    R --> T
    Q --> T
    T --> S
    S --> W["worker<br/>做最小修改"]
    W --> V["reviewer / security-reviewer<br/>独立审查"]
    V --> S
    S --> A{"是否涉及高风险变更?"}
    A -->|"是"| H["用户审批"]
    A -->|"否"| F["最终验证与总结"]
