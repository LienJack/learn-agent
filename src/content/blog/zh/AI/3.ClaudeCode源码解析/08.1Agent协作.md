---
title: 8.1 Agent 协作：Claude Code 如何从单线程助手变成多 Agent 运行时
...
```mermaid
flowchart TD
    U["用户请求：修复登录失败"] --> M["主 Agent 进入 ReAct 循环"]
    M --> R["读取 auth 相关文件"]
    R --> D{"是否需要局部深挖?"}
    D -->|"是"| A["调用 AgentTool"]
    A --> S["选择 subagent_type / model"]
    S --> P["组装子 Agent prompt"]
    P --> T["裁剪工具池与权限上下文"]
    T --> X["启动子 Agent"]
    X --> W["子 Agent 搜索、读取、分析"]
    W --> O["返回压缩摘要"]
    O --> M2["主 Agent 合并结论"]
    M2 --> N["继续修改、验证或再派发任务"]
```
...
```mermaid
flowchart TD
    A["AgentTool<br/>run_in_background=true"] --> B["registerAsyncAgent / registerRemoteAgentTask"]
    B --> C["AppState.tasks 写入 local_agent / remote_agent"]
    C --> D["输出写入任务 output file"]
    D --> E["TaskOutput 读取输出"]
    C --> F["TaskStop 停止任务"]
    C --> G["UI foreground / viewingAgentTaskId 查看转录"]
    E --> H["主 Agent 继续合并结果"]
```
...
```mermaid
flowchart TD
    M["主 Agent 模型输出 tool_use"] --> T{"工具类型"}
    T -->|"AgentTool"| A["启动子 Agent"]
    T -->|"TaskOutput"| O["读取后台任务输出"]
    T -->|"TaskStop"| K["停止后台任务"]
    T -->|"SendMessage"| S["向 Agent / teammate / bridge 发消息"]
    T -->|"AskUserQuestion"| Q["向用户请求决策"]
    A --> R["注册 local_agent / remote_agent task"]
    S --> B["mailbox / route / structured message"]
    Q --> U["用户回复进入后续上下文"]
    R --> C["AppState.tasks 更新"]
    O --> M2["结果回注主 Agent"]
    B --> M2
    U --> M2
    M2 --> N["下一轮 ReAct"]
...
- 本地 wiki：[Claude Code Agent 协作机制总览](../../../wiki/AI/claude-code-agent-协作机制总览.md)
- 本地 wiki：[Deep Dive Claude Code Ch10：Agent 模型](../../../wiki/AI/deep-dive-claude-code-ch10-agent-model.md)
- 本地 wiki：[Deep Dive Claude Code Ch11：子 Agent 编排](../../../wiki/AI/deep-dive-claude-code-ch11-subagent.md)
- 本地 wiki：[Claude Code Task 源码设计：任务清单与运行时任务](../../../wiki/AI/claude-code-task-源码设计-任务清单与运行时任务.md)
- 本地 wiki：[第08章 多 Agent 协作、Teams 与权限冒泡](../../../wiki/AI/第08章-多-agent-协作、teams-与权限冒泡.md)
- 原始材料：[AgentTool：子 Agent 调度器](../../../raw/AI/claude-code-xuanyuan/2026-05-01-agenttool-subagent-dispatcher.md)
- 原始材料：[SendMessageTool：Agent 通信](../../../raw/AI/claude-code-xuanyuan/2026-05-01-sendmessagetool-agent-messaging.md)
- 原始材料：[多 Agent 与子任务机制](../../../raw/AI/claude-code-xuanyuan/2026-05-01-multi-agent-and-subtasks.md)
