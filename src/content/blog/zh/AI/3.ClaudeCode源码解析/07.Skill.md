---
title: 7.Skill：Claude Code 如何把经验封装成可加载的能力包
...
```mermaid
flowchart TD
    A["启动 / 文件访问 / 插件加载"] --> B["扫描 skill 来源"]
    B --> C["读取 SKILL.md frontmatter"]
    C --> D["生成轻量 Skill 索引<br/>name + description + when_to_use"]
    D --> E["注入本轮上下文<br/>告诉模型有哪些 Skill 可用"]
    E --> F{"用户请求是否匹配某个 Skill?"}
    F -->|"不匹配"| G["普通 ReAct / Tools 流程"]
    F -->|"匹配"| H["渲染 Skill 内容"]
    H --> I["参数替换<br/>动态命令注入<br/>支持文件按需读取"]
    I --> J{"context: fork ?"}
    J -->|"否"| K["inline 注入当前会话<br/>成为持续有效的任务说明"]
    J -->|"是"| L["创建 fork 子 Agent<br/>隔离上下文执行任务"]
    K --> M["模型按 Skill 指南调用 Tools"]
    L --> N["子 Agent 执行后返回摘要"]
    M --> O["结果进入消息历史"]
    N --> O
    O --> P["后续回合继续 ReAct 循环"]
...
```mermaid
flowchart LR
    M["MCP<br/>外部能力接入协议"] --> T["Tools / Resources / Commands"]
    B["Built-in Tools<br/>Read / Grep / Bash / Edit"] --> T
    T --> S["Skill<br/>任务方法与执行规范"]
    S --> Q["QueryEngine<br/>按上下文推进任务"]
    Q --> R["模型调用工具<br/>执行 / 回填 / 再判断"]
```
...
```mermaid
sequenceDiagram
    participant FS as 文件系统 / 插件 / 管理策略
    participant Loader as Skill Loader
    participant Prompt as Context Builder
    participant Model as Model
    participant Runtime as Skill Runtime
    participant Tools as Tools / Subagent

    FS->>Loader: 提供 .claude/skills/*/SKILL.md
    Loader->>Loader: 解析 frontmatter / 去重 / 处理优先级
    Loader->>Prompt: 输出轻量 Skill 索引
    Prompt->>Model: 注入 name + description
    Model->>Runtime: 选择或响应 /skill-name
    Runtime->>Runtime: 渲染 SKILL.md / 参数替换 / 动态注入
    alt inline skill
        Runtime->>Model: 把完整 Skill 内容注入当前会话
        Model->>Tools: 按说明调用工具
    else context: fork
        Runtime->>Tools: 创建子 Agent 并传入 Skill 内容
        Tools->>Model: 返回子 Agent 汇总
    end
```
...
```mermaid
flowchart TB
    U["用户任务"] --> Q["QueryEngine 主循环"]
    Q --> C["Context Builder<br/>system prompt / memory / skill index"]
    C --> M["Model<br/>理解任务并选择行动"]
    M -->|"tool_use"| T["Tools<br/>Read / Grep / Bash / Edit"]
    M -->|"skill invocation"| S["Skill Runtime<br/>加载并渲染 SKILL.md"]
    M -->|"mcp tool_use"| MCP["MCP Wrapped Tools<br/>外部服务能力"]
    S -->|"inline"| C
    S -->|"context: fork"| A["Subagent<br/>隔离执行并返回摘要"]
    T --> Q
    MCP --> Q
    A --> Q
```
...
- [Claude Code 源码专题：扩展能力（MCP、Skills、多 Agent 与远程）](../../../wiki/AI/claude-code-%E6%BA%90%E7%A0%81%E4%B8%93%E9%A2%98-%E6%89%A9%E5%B1%95%E8%83%BD%E5%8A%9B-mcp-skills-%E5%A4%9Aagent%E4%B8%8E%E8%BF%9C%E7%A8%8B.md)
- [Deep Dive Claude Code Ch12：Skill 系统](../../../wiki/AI/deep-dive-claude-code-ch12-skill-system.md)
- [Agent 概念祛魅：从 Prompt、Memory、RAG、MCP 到 Skill](../../../wiki/AI/agent-%E6%A6%82%E5%BF%B5%E7%A5%9B%E9%AD%85-%E4%BB%8E-prompt-memory-rag-mcp-%E5%88%B0-skill.md)
