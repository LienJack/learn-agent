---
title: 1.工程架构：Claude Code 不止是一个会写代码的聊天框
...
# 1.工程架构：Claude Code 不止是一个会写代码的聊天框
...
![](./assets/claude-code-functional-architecture.svg)
...
flowchart TB
    ...
    subgraph "Agent 协作层"
        AGENT["Agent / Task<br/>子 Agent 派发与结果聚合"]
    end

    subgraph "扩展能力层"
        MCP["MCP<br/>外部工具与资源"]
        LSP["LSP<br/>代码语义能力"]
        SKILLS["Skills<br/>可复用任务方法"]
    end

    subgraph "上下文与状态层"
        CTX["Context / Memory<br/>动态上下文装配与压缩"]
        STATE["AppStateStore<br/>全局状态管理"]
    end

    subgraph "工具层"
        TOOLS["Tools 系统<br/>文件 / Shell / 搜索 / Web"]
    end
...
sequenceDiagram
    participant U as 用户
    participant REPL as REPL / CLI UI
    participant QE as QueryEngine
    participant CTX as Context / Memory
    participant STATE as AppStateStore
    participant MODEL as Model API
    participant PERM as 权限系统
    participant TOOLS as Tools / MCP / LSP / Agent
    participant WORLD as 工程环境

    U->>REPL: 输入任务
    REPL->>QE: submitMessage(prompt)
    QE->>STATE: 读取模式 / 权限 / 工具状态
    QE->>CTX: 装配 system prompt / 历史 / 项目规则
    QE->>MODEL: 发送消息与可用工具说明
    MODEL-->>QE: 返回文本或 tool_use
    ...
    alt 普通回答
        ...
        alt 允许执行
            PERM-->>QE: allow
            QE->>TOOLS: 执行工具
            TOOLS->>WORLD: 读文件 / 跑命令 / 调 MCP / 派发 Agent
            WORLD-->>TOOLS: 返回结果
            TOOLS-->>QE: 工具结果
            QE->>CTX: 写回消息历史 / 必要时压缩
            QE->>MODEL: 带工具结果继续下一轮
...
```mermaid
flowchart TB
    CLI["entrypoints/cli.tsx\n二进制入口 / 快速路径"] --> MAIN["main.tsx\n参数解析 / 配置 / 环境初始化"]

    MAIN --> REPL["REPL & 终端 UI\n输入 / 流式输出 / 权限确认"]
    MAIN --> HEADLESS["Headless / SDK\n非交互调用路径"]
    MAIN --> MCPServe["mcp serve\nMCP 服务入口"]

    REPL --> QE["QueryEngine\n会话生命周期 / submitMessage"]
    HEADLESS --> QE

    QE --> Commands["commands.ts\n斜杠命令 / 本地命令"]
    QE --> Query["query 主循环\n模型调用 / ReAct Loop"]
    QE --> Context["context.ts / prompt assembly\n上下文注入"]
    QE --> State["state/AppStateStore\n全局状态管理"]
    QE --> Tools["tools.ts / Tool.ts\n工具注册 / 工具契约"]

    Tools --> Builtin["内置工具\nRead / Edit / Bash / Grep / Glob"]
    Tools --> ToolExec["Tool Executor\n权限 / 并发 / 结果序列化"]
    Tools --> MCP["services/* MCP\n外部工具与资源"]
    Tools --> LSP["LSP\n代码语义能力"]
    Tools --> Skills["Skills\n可复用任务方法"]
    Tools --> AgentTool["Agent / Task\n子 Agent 协作"]

    State --> UIState["UI 状态"]
    State --> PermissionState["权限上下文"]
    State --> TaskState["Task / Agent 状态"]
    State --> ExtensionState["MCP / 插件状态"]

    Context --> Memory["Memory / Compression\n历史摘要 / 上下文压缩"]
    Query --> Model["Model API"]
    ToolExec --> Env["文件系统 / Shell / 网络 / 外部服务"]
```
