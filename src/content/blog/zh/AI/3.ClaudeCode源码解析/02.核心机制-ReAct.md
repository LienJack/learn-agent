---
title: 2.核心机制-ReAct：query.ts 如何把模型变成会行动的 Agent
...
```mermaid
flowchart TD
    A["思考：我现在知道什么，还缺什么？"] --> B["计划：下一步应该做什么？"]
    B --> C["执行：调用工具接触真实环境"]
    C --> D["反馈：拿到文件、命令、搜索或接口结果"]
    D --> E["再思考：基于新事实更新判断"]
    E --> B
    E --> F{"信息足够了吗？"}
    F -- "否" --> B
...
```mermaid
flowchart TD
    A[构建 Query] --> B[请求 Model API]
    B --> C[解析返回结果]
    C --> D{有工具调用?}

    D -- 否 --> E[break\n返回结果]
    D -- 是 --> F[调用工具]
    F --> G[6. 工具结果追加到 messages]
    G --> H[7. 检查是否需要压缩]
    H --> A
...
```mermaid
flowchart TD
    QE["QueryEngine 会话级编排器"] --> SM["submitMessage 启动 Agent Run"]
    SM --> A["当前 State"]
    A --> B["构建 Query"]
    B --> C["请求 Model API"]
    C --> D["解析返回结果"]
...
- [Claude Code 核心引擎：QueryEngine 如何驱动一次任务完成](https://www.xuanyuancode.com/learn-claude-code/tutorials/cc5)
- 本地 wiki：[query.ts 状态机：构建 Query、工具调用与压缩闭环](../../../wiki/AI/query-ts-%E7%8A%B6%E6%80%81%E6%9C%BA%EF%BC%9A%E6%9E%84%E5%BB%BA-query%E3%80%81%E5%B7%A5%E5%85%B7%E8%B0%83%E7%94%A8%E4%B8%8E%E5%8E%8B%E7%BC%A9%E9%97%AD%E7%8E%AF.md)
