---
title: 3.核心机制-Prompt 编写：Claude Code 如何动态组装模型的工作台
...
```mermaid
flowchart TB
    subgraph S["System prompt：系统层"]
        S1["稳定规则（cached）<br/>身份、系统规则、工具使用指南、安全边界"]
        S2["动态片段<br/>Agent / Skills / MCP / Git 状态 / 权限模式"]
    end

    subgraph U["User prompt：用户与任务层"]
        U1["记忆与项目规则<br/>CLAUDE.md、用户偏好、项目规范"]
        U2["当前用户消息<br/>本轮真正要解决的问题"]
    end

    subgraph M["Messages：运行现场"]
        M1["历史对话"]
        M2["工具调用与工具结果"]
        M3["压缩摘要"]
    end
...
```mermaid
flowchart TD
    A{"有 overrideSystemPrompt？"} -- "是" --> P0["使用 overrideSystemPrompt<br/>完全替换主提示词"]
    A -- "否" --> B{"当前是 Coordinator？"}
    B -- "是" --> P1["使用 Coordinator prompt"]
    B -- "否" --> C{"当前是自定义 Agent？"}
    C -- "是" --> P2["使用 Agent prompt"]
    C -- "否" --> D{"有 customSystemPrompt？"}
    D -- "是" --> P3["使用 customSystemPrompt"]
    D -- "否" --> P4["使用 defaultSystemPrompt"]
    P0 --> E["追加 appendSystemPrompt"]
    P1 --> E
    P2 --> E
    P3 --> E
    P4 --> E
    E --> F["得到 effective system prompt"]
```
...
```mermaid
flowchart TB
    M["Managed memory<br/>/etc/claude-code/CLAUDE.md<br/>组织或管理员规则"]
    U["User memory<br/>~/.claude/CLAUDE.md<br/>用户长期偏好"]
    P["Project memory<br/>CLAUDE.md / .claude/CLAUDE.md / .claude/rules/*.md<br/>项目共享规范"]
    L["Local memory<br/>CLAUDE.local.md<br/>本地私有配置"]
    C["user context<br/>进入本轮上下文"]
...
```mermaid
flowchart LR
    subgraph Prefix["稳定前缀：适合 Prompt Cache"]
        A["身份介绍"]
        B["系统规则"]
        C["工具使用指南"]
        D["安全边界"]
    ...
    Boundary["动态边界<br/>SYSTEM_PROMPT_DYNAMIC_BOUNDARY"]

    subgraph Dynamic["动态段：按轮次刷新"]
        E["CLAUDE.md / 记忆"]
        F["Git 状态 / 当前日期"]
        G["Skills / MCP / Agent 上下文"]
        H["权限模式 / 压缩摘要"]
    end

    Prefix --> Boundary --> Dynamic --> Q["本轮请求"]
```
...
```mermaid
flowchart TD
    A["选择 system prompt 主规则"] --> B["追加必要补充规则"]
    B --> C["加载 CLAUDE.md 分层记忆"]
    C --> D["注入动态 system/user context"]
    D --> E["准备 toolUseContext"]
    E --> F["合并 messages 与当前用户输入"]
    F --> G["请求 Model API"]
    G --> H{"模型是否调用工具？"}
    H -- "否" --> I["返回最终回答"]
    H -- "是" --> J["执行工具并得到结果"]
...
- [Claude Code Docs Chapter 04: context engineering](../../../wiki/AI/claude-code-docs-chapter-04-context-engineering.md)
- [第07章 Prompt Runtime、上下文压缩与缓存边界](../../../wiki/AI/第07章-prompt-runtime、上下文压缩与缓存边界.md)
- [Claude Code 运行流程图与步骤详解](../../../wiki/AI/claude-code-运行流程图与步骤详解.md)
- [Deep Dive Claude Code Ch05：消息系统](../../../wiki/AI/deep-dive-claude-code-ch05-message-system.md)
- [Deep Dive Claude Code Ch18：会话管理与压缩](../../../wiki/AI/deep-dive-claude-code-ch18-conversation.md)
