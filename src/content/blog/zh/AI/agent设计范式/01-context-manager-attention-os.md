...
如果前面的 [Context Policy](/blog/AI/build-harness/00-15-context-policy-model-input) 回答的是：
...
```text
Context Manager 的职责不是保存对话。
它的职责是在任何时刻重建 Agent 做出下一步行动所需的最小充分上下文。
```
...
## 三、新范式：Context 是编译产物，不是事实源

![Context Manager 新范式：Agent 的注意力操作系统 - 三、新范式：Context 是编译产物，不是事实源](./assets/01-context-manager-attention-os/photo-03-context.png)
...
```text
1. 上下文不是历史，而是行动所需的信息环境。
2. 事实源和派生物必须分离。
3. 所有有损变换必须可审计。
4. State 是投影，不是聊天记录。
5. Context 是编译结果，不是数据库。
6. Memory 是治理后的知识，不是缓存。
7. 工具调用是因果事件，不是普通文本。
8. 推理链路保存可解释骨架，而不是隐藏思维链。
9. 上下文必须有作用域。
...
但它应该是 typed message blocks，而不是 provider 原始格式的透传数组。
...
`Memory` 回答的是哪些知识值得未来复用。它不是“把所有 summary 丢进向量库”。
...
![Context Manager 新范式：Agent 的注意力操作系统 - Trace：可解释链路，不是隐藏思维链](./assets/01-context-manager-attention-os/photo-10-trace.png)
...
## 七、Compaction：语义蒸馏，不是文本缩短

![Context Manager 新范式：Agent 的注意力操作系统 - 七、Compaction：语义蒸馏，不是文本缩短](./assets/01-context-manager-attention-os/photo-12-compaction.png)
...
![Context Manager 新范式：Agent 的注意力操作系统 - 十四、和前面几章的关系](./assets/01-context-manager-attention-os/photo-17-item-cc93ad2b.png)
