---
title: "Capability Discovery：按任务暴露最小能力集合"
description: "当工具、Skills、MCP 和插件越来越多时，用 Capability Catalog、Discovery Policy 和 Visible Set 治理模型本轮能看见的能力。"
author: LienJack
pubDate: 2026-05-29
...
# Capability Discovery：按任务暴露最小能力集合

沿着前面的实现继续加功能，一个很自然的冲动会出现：
...
一个真实项目里，模型 prompt 可能突然出现 100 个工具、20 个 Skill、5 个 MCP server、十几种资源和 prompt 模板。模型不是在 UI 里慢慢浏览菜单，它是在一次模型输入里做下一步决策。
...
![Capability Discovery：按任务暴露最小能力集合 Mermaid 1](assets/00-17-capability-discovery-skills-mcp/mermaid-01.png)

看这张图时，先看 `Capability Catalog` 和 `Visible Set` 中间那道门。
...
## 二、先拆能力类型：tool、skill、resource、prompt
...
![Capability Discovery：按任务暴露最小能力集合 Mermaid 2](assets/00-17-capability-discovery-skills-mcp/mermaid-02.png)
...
![Capability Discovery：按任务暴露最小能力集合 Mermaid 3](assets/00-17-capability-discovery-skills-mcp/mermaid-03.png)
...
![Capability Discovery：按任务暴露最小能力集合 Mermaid 4](assets/00-17-capability-discovery-skills-mcp/mermaid-04.png)
...
![Capability Discovery：按任务暴露最小能力集合 Mermaid 5](assets/00-17-capability-discovery-skills-mcp/mermaid-05.png)

看这张图时，别停在 `ToolSearch`，还要看它后面的 `权限和预算`。
...
![Capability Discovery：按任务暴露最小能力集合 Mermaid 6](assets/00-17-capability-discovery-skills-mcp/mermaid-06.png)
...
![Capability Discovery：按任务暴露最小能力集合 Mermaid 7](assets/00-17-capability-discovery-skills-mcp/mermaid-07.png)
...
```text
disabled capability 不进入 visible set。
ToolSearch 只返回 candidate，不直接授权。
Skill 全文只在命中后加载，不常驻 system prompt。
MCP server 连接成功不等于全量工具进入模型输入。
full schema 只在 visible 后注入。
```

验收标准是：

```text
registry 里有很多能力，模型本轮只看到最小可用集合。
visible set 的变化有 diff，可审计、可 replay。
找到能力、看见能力、执行能力是三件事。
```
