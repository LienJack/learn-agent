...
更准确地说，`Model / Loop / Tools / State` 更像四条责任边界，不只是四个漂亮名词。

很多 Agent 代码写着写着会乱，根本原因通常不在模块名，而在责任混在了一起：
...
![展示 Model、Loop、Tools、State 在闭环里的四条责任边界](assets/00-02-agent-components/photo-01-four-part-agent-loop.jpg)
...
这就是为什么模型接口最好一开始就按“事件”或“意图”来设计，别只返回一段字符串。
...
这里先放下 TypeScript 细节，重点看边界：provider 只产出模型事件，不执行工具。工具执行属于 Runtime。
...
这些能力可以写，但不能放在 provider 这一层。它们应该属于 Runtime、Tool Runtime、Session Store 或 Context Builder。

这里还有一个细节：模型输出的 `tool_intent` 最好被当成“不可信建议”，别当成命令。
...
模型越强，这条边界越重要。因为强模型更会规划，也更会“合理化”危险动作。Harness 要让能力通过可检查的协议落地，并不压制模型能力。
...
这六个阶段的价值，是给后续机制留插槽，而不是把代码写得很有仪式感。
...
因为模型可能同时输出文本和工具意图，可能输出多个工具意图，可能输出格式不完整的工具意图，可能在权限被拒后继续请求同一个工具。Loop 的工作是把模型事件解释成下一步运行时决策，而不是盲目执行：
...
一旦有了这层 decision，后面的能力就有地方挂了。权限进入 `request_approval`，压缩进入 `compact`，重复失败进入 `repair` 或 `fail`，不用散落在工具函数或临时分支里。
...
所以 Loop 的核心价值，是把每一轮行动放进可治理的生命周期里，而不只是“循环”。
...
这只是实现细节，还够不上工具协议。工具协议还要描述它的名字、用途、schema、风险等级、是否只读、是否允许并发、输出预算、错误如何展示。

第二个误区是把权限放到执行之后才想。

正确顺序应该是先判断模型本轮能不能看到这个工具，再判断这次具体调用能不能执行。能见度和执行权是两道门，不能混在一起。

这里的“可见性”属于安全边界，不是 UI 优化。
...
候选工具池属于产品能力。可见工具集属于上下文构建和策略裁剪。可执行调用属于权限判定。审计日志要记录最终发生过什么，不能只停在模型“本来想做什么”。
...
Runtime 管的是这些部件如何协作，而不是某一个单独部件：
...
这也是整套教程后续的主线：先观察每个部件遇到的真实问题，再把必要的控制层补出来，而不是一开始设计一个巨大架构。
...
这些目录不是为了显得架构完整；它们分别分担了四件套变复杂后的压力。
...
教学项目可以把四个部件直接落成四组文件：`MockModel` 代表 Model，`runAgentLoop()` 代表 Loop，`ToolRegistry` 代表 Tools，`JsonlSessionStore` 代表 State。关键在于数据流要单向清楚，不在名字是否一一对应：model 只能生成 message 或 tool intent，loop 负责推进，tool runtime 负责副作用，store 负责从历史构造下一轮 context。
