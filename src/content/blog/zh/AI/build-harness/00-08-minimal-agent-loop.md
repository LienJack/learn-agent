...
description: "前面几篇我们一直在拆一个问题：Agent 靠受控过程持续推进任务，不能只靠更长的 Prompt 或更会聊天的模型。"
author: LienJack
pubDate: 2026-05-29
...
前面几篇我们一直在拆一个问题：Agent 靠受控过程持续推进任务，不能只靠更长的 Prompt 或更会聊天的模型。
...
单次回答的瓶颈不在“不聪明”，而在它没有机会去拿这些事实。

它只能在缺事实的状态下给建议。

Agent Loop 做的第一件事，就是承认模型第一轮不知道答案。

这很重要。一个能行动的 Agent 要允许自己先说：

```text
我需要先观察。
```

对于“修复测试失败”这个任务，第一轮合理的判断通常是：
...
模型先提出“我要看什么”，系统执行这个动作，拿到观察结果，再让模型继续判断。
...
那已经变成一个风险很高的模型遥控器，够不上 Agent Loop。
...
因为最小 Agent Loop 的核心在于每轮如何更新 state，而不在 `while` 本身。
...
最小 Agent 的第一原则是：每增加一个能力，都要知道它挂在哪条状态转移上。
...
Observation 是“工具执行结果的模型可读摘要 + 必要证据”，不是原始日志的复制。
...
**Observe 要把真实世界整理成下一轮判断所需的事实，而不是直接把工具输出丢给模型。**
...
这还够不上最小 Agent Loop，只是靠字符串暗号拼出来的流程。
...
修法是把 observation 回填做好，不是单纯“让模型更聪明”。
...
第一，Loop 要让观察结果改变下一轮判断，而不是只让模型多说几轮。
...
写代码时，只要守住这条线：Agent Loop 的本质，是让模型在受控状态机里反复用真实观察修正下一步，避免变成无限续写回答。

下一篇会继续沿着这条线往下走：当真实大模型被接进系统时，core 应该如何保持控制权？也就是说，我们要把模型接进 loop，同时把控制权留在 loop 里。
