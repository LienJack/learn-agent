...
写到这里，我们的小型 CLI Agent 已经不再是最早那个能跑一次的 demo。
...
它有 capability discovery。

它有 session replay。

它知道模型只能提出 intent。

它知道工具必须经过 validate、permission、execute、observe。

它也知道外部能力不能绕过统一 tool pipeline。

如果你只是在本机给自己做一个实验工具，到这一步已经很不错。
...
Agent 可以跑测试、读文件、搜索错误、修改代码、再跑测试。

它像一个小型 Claude Code 的雏形。
...
> 一个 demo CLI 只要能跑一次；一个产品化 CLI 必须把 profile、配置层、provider 切换、extension 安装、能力发现、项目指令、运行检查和稳定输出协议放进同一套 Harness 纪律里。
...
```text
Productized CLI 不是把 demo 包成 npm 包。
Productized CLI 是把 runtime 能力投影成可配置、可扩展、可检查、可替换、可被 host 消费的产品入口。
```
...
## 四、multi-provider：不能让 provider 细节穿透用户体验
...
这张图里最重要的是两个 provider 最后都汇入 `ModelEvent`。
...
## 五、extension：安装不是启用，启用不是可见，可见不是可执行
...
Productized CLI 解决的不是“Agent 会不会思考”。

前面的文章已经解决了模型、loop、tool、context、session、capability、delegation。

这一篇解决的是：

```text
这些 runtime 能力如何以稳定产品入口暴露给真实用户和 host。
```

它把散落的 CLI flags 收束成 profile。

它把 provider 切换收束成 resolver。

它把 extension 安装收束成 manifest、trust、catalog、discovery 和 permission。

它把项目指令收束成 context source。

它把终端输出收束成 event stream 和 renderer。
...
第二，profile 可能被滥用成大杂烩。

所以 profile 只表达运行身份，不执行能力。

第三，multi-provider 会带来能力差异。

所以 provider capability 必须用内部语义表达。

第四，extension 会带来信任问题。

所以安装、启用、可见、可执行必须分开。

第五，host/workbench 需要稳定协议。

所以 Runtime 事件和 Renderer 必须分开。

这正好引出下一篇。

当 CLI 已经能作为产品入口运行，下一步就不是继续往本地 CLI 里堆能力。

下一步是把 Harness 放到更远的环境里：

```text
Sandbox、Cron、durable execution 与远程部署。
```

也就是 Hosted Harness。

到那时，profile 会变成远程任务的运行身份。

extension trust 会变成部署边界。

provider resolver 会变成调度策略。

event stream 会变成远程可观察性协议。

而本篇的 Productized CLI，正是进入 Hosted Harness 之前最后一层本地入口纪律。
