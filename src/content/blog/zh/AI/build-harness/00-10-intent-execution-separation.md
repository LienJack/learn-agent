...
这个想法看起来没有问题。毕竟 Agent 的魅力就在这里：它不只是聊天，它能行动。

但真正危险的地方也在这里。

因为模型输出的本质仍然是概率性文本。哪怕它现在可以通过 function calling 或 structured output 生成 JSON，那个 JSON 也只是模型在当前上下文里提出的下一步建议。它不是授权。不是事实。不是已经执行过的动作。也不是系统必须照做的命令。

如果我们把模型输出直接接到文件系统、shell、数据库、浏览器、支付接口或远程 API 上，一个小型 CLI Agent 很快就会变成这样：
...
模型说：编辑 src/auth.ts
系统执行：覆盖 src/auth.ts
模型说：清理 node_modules 后重装依赖
系统执行：rm -rf node_modules && npm install
模型说：测试还是失败，重置仓库
系统执行：git reset --hard
```

表面看它很主动，实际上系统已经失去了最关键的控制权：**谁把意图变成动作，谁就要为外部世界的变化负责。**
...
这个 Agent 会慢慢拥有读取文件、搜索代码、编辑文件、执行测试、查看 Git 状态的能力。但第 10 篇不急着把工具系统完整做出来。我们先钉住一条更底层的工程纪律：
...
![用一条横向管线解释模型意图如何经过校验、审批、执行和观察回填](assets/00-10-intent-execution-separation/photo-01-intent-execution-pipeline.png)
...
## 二、Intent 不是一句自然语言，而是系统可处理的请求对象

![强调模型提交的是申请单，真正接触文件系统和 shell 的是 Runtime](assets/00-10-intent-execution-separation/photo-02-model-runtime-boundary.png)
...
## 四、Approve：权限不是弹窗，而是 intent 与 execution 之间的闸门

![画出工具可见性和单次审批两道门，说明权限不是最后的弹窗](assets/00-10-intent-execution-separation/photo-03-permission-gates.png)
...
![展示完整事件事实链如何同时服务模型观察、审计和重放](assets/00-10-intent-execution-separation/photo-04-observation-audit-replay.png)
...
Intent / Execution 分离不是抽象架构洁癖。

它是在 Agent 接触真实世界之前，必须立住的第一条工程纪律。
