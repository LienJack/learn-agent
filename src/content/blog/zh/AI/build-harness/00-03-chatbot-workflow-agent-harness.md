...
![用不确定性位置来判断 ChatBot、Workflow、Agent、Harness 的边界](assets/00-03-chatbot-workflow-agent-harness/photo-01-boundary-decision-map.jpg)
...
这些问题都不属于模型本身，也不属于单个工具。它们属于 Agent 外部的工程控制系统，也就是 Harness。
...
如果答案已经走到“模型动态决定下一步、有真实副作用、失败后必须恢复和审计”，就进入 Harness 问题。后面会用 ETCLOVG 这七层展开它的责任范围：
...
本篇只列名字，下一篇专门拆开。但这里要先记住一点：这些都不是模型内部能力。它们是模型外面的工程责任。
...
![突出 Workflow 的控制权在代码，Agent 的控制权部分交给运行时模型判断](assets/00-03-chatbot-workflow-agent-harness/photo-02-workflow-vs-agent-control.jpg)
...
![把不确定性、风险、控制成本画成走向 Harness 的压力图](assets/00-03-chatbot-workflow-agent-harness/photo-03-uncertainty-budget-to-harness.jpg)
...
教学项目里可以保留两个入口来体现边界：`POST /api/prompt` 是一次性调试入口，`/api/runs` 加事件流更接近 Harness 运行入口。固定流程适合写在 API 或测试里，模型只在需要不确定判断时进入 loop。这样读者会知道：不是所有自动化都要升级成 Agent，只有“下一步取决于观察结果”时才需要 Agent Loop。
