...
**Agent 的关键在于把模型、循环、工具和状态组织成一个运行过程；进入真实环境后，还需要外部 Harness 托管它。**
...
在 Claude Code 这类编程 Agent 里，这道缝尤其明显。用户把一个真实项目交给 Agent，让它在文件系统、终端、Git、测试框架、项目规则、权限边界之间来回移动，目标早已超出“解释一段代码”。模型本身没有这些能力，它只是每一轮根据上下文判断下一步。
...
Prompt 只影响模型如何生成回答。ChatBot 开始管理多轮对话。Agent 加入行动循环。Harness 则把行动放进工程边界里，让权限、日志、测试和恢复机制为它托底。
...
这可以跑通最小演示，但离可长期使用的系统还差很远。因为真实任务里最难的是：
...
所以做 Agent 的第一步，是让系统先分清楚三件事，而不是让模型说得更像工程师：
...
如果再往工程实现压一层，这个 loop 其实是一组事件边界，不能变成“模型想做什么就做什么”：
...
这张图不要求 Agent 一定写成这么多类。它是在提醒我们：每个箭头都可能失败，每个失败都需要被记录成下一轮可见的状态。
...
这也是为什么“给模型接一个 shell”只是风险的起点，远远不是 Agent 工程的终点。
...
这里有一个非常重要的细节：工具调用本身是“请求执行”，还不是“执行”。
...
所以从工程视角看，Agent 的可靠性更多依赖一个受控运行时，不能只依赖人设和 prompt。

Claude Code 这类系统真正值得学习的地方，也正是在这里：它把模型放进一个有工具协议、权限边界、上下文调度、压缩、审计和恢复能力的工程壳里，避免把模型神化。
...
这里的箭头不表示“必须一路升级”。它只是提醒读者：任务越接近真实环境，模型外部责任越多；如果问题只需要对话或确定流程，就不必强行 Agent 化。
...
```text
ReAct 负责循环推进，别把它理解成神秘推理术。
Tool Use 把行动意图协议化，别理解成给模型超能力。
Context Engineering 决定本轮该给模型什么事实，重点不在写长 prompt。
Memory 跨任务保留可复用经验，不能只是保存聊天记录。
Evaluation 防止 Harness 改坏已有能力，不只是事后打分。
```
...
把这章落到教学项目里，第一步是让 prompt 只表达角色和边界，把“行动”交给 `runAgentLoop()` 与 `ToolRegistry`，而不是写更长的 system prompt。验收点也应该很朴素：用户说“列出工作区文件”时，系统必须产生 assistant 的 `toolCall`、工具的 `toolResult`，再由 assistant 基于结果回答。这样读者一开始就能看到：prompt 给方向，Agent Loop 组织过程，Harness 负责执行和记录。
