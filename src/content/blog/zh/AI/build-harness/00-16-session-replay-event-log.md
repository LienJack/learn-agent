---
title: "Session Replay：用事件日志恢复长任务现场"
description: "从进程崩在 ToolStarted 后的事故出发，解释事件日志、artifact、snapshot、ReplayRunner 和 ResumeGate 如何恢复现场。"
author: LienJack
pubDate: 2026-05-29
...
# Session Replay：用事件日志恢复长任务现场

Agent 正在执行 `edit_file`。
...
```text
长任务中断以后，系统如何从事件日志恢复现场，并判断能不能继续？
```
...
![Session Replay：用事件日志恢复长任务现场 Mermaid 1](assets/00-16-session-replay-event-log/mermaid-01.png)
...
![Session Replay：用事件日志恢复长任务现场 Mermaid 2](assets/00-16-session-replay-event-log/mermaid-02.png)
...
![Session Replay：用事件日志恢复长任务现场 Mermaid 3](assets/00-16-session-replay-event-log/mermaid-03.png)
...
![Session Replay：用事件日志恢复长任务现场 Mermaid 4](assets/00-16-session-replay-event-log/mermaid-04.png)
...
![Session Replay：用事件日志恢复长任务现场 Mermaid 5](assets/00-16-session-replay-event-log/mermaid-05.png)
...
![Session Replay：用事件日志恢复长任务现场 Mermaid 6](assets/00-16-session-replay-event-log/mermaid-06.png)
...
```text
replay 不调用模型、不执行工具。
snapshot 与 event log 冲突时，以 event log 为准。
停在 ToolStarted / EditStarted 后，不能自动重跑写操作，必须进入 Resume Gate。
```
...
```text
只保存 messages 不能算通过。
重启后能重建 state、pending action 和最后稳定点。
继续行动前能解释：为什么可以继续，或者为什么必须停下来让用户确认。
```
