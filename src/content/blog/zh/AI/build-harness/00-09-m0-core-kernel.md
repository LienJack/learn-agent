---
title: "M0 Core Kernel：真实模型接入系统边界"
...
# M0 Core Kernel：真实模型接入系统边界

Fake provider 能把 loop 跑通，但它暴露不了真实模型接入的麻烦。

真实 provider 一接进来，就会出现 stream delta、tool call fragment、usage、request id、rate limit、context length error、provider-specific stop reason。

M0 要做的是先把这些外部事件落成本项目自己的内部事件，而不是急着让模型更能干。

很多项目会先把 OpenAI、Anthropic、Gemini 或任意一个 provider 的 API 调通。能流式输出。能解析工具调用。能在终端里打印回答。再顺手把工具执行也接进去。
...
这条路短期很爽。隐蔽的问题是：系统中心很容易从 `runtime` 滑向 `provider`。

一开始只是 provider 返回一段文本。后来 provider 返回 tool call。再后来 provider 的响应格式决定了工具对象长什么样。再后来错误处理、streaming、messages、tool results、上下文结构都跟 provider 绑在一起。最后你会发现，整个 Agent core 已经围着某一家模型 API 的返回格式运转，自己的 contracts 反而退到了后面。
...
沿用第 7 篇的 provider contract 和第 8 篇的 loop。到了第 9 篇，问题变成：

> 当真实大模型开始返回文本、流式事件和 tool intent 时，core 要如何把它纳入自己的边界，避免被 provider 响应格式牵着走？
...
这一章先留下一个边界：M0 Core Kernel 的职责，是把真实模型输出翻译成系统内部事件，并让 runtime 继续拥有执行、状态和日志的控制权。
...
![M0 Core Kernel：真实模型接入系统边界 Mermaid 1](assets/00-09-m0-core-kernel/mermaid-01.png)
...
如果这两条边界守住，真实模型接入只需要替换 provider adapter，不会变成重写 core。
...
M0 的目标是给后面所有层提供一个不晃的底座，不追求“一步到位”。

可以把 M0 看成一个很小的控制面：

![M0 Core Kernel：真实模型接入系统边界 Mermaid 2](assets/00-09-m0-core-kernel/mermaid-02.png)
...
原因不在接口名字好看，而在后面的权限、回放、压缩、评估都要挂在这些对象上。
...
![M0 Core Kernel：真实模型接入系统边界 Mermaid 3](assets/00-09-m0-core-kernel/mermaid-03.png)
...
如果模型返回错误，runtime 应该把错误变成可归因事件，避免异常直接炸穿整个 loop。

这就是“真实模型接入系统边界”的第一层落地。
...
Core 需要一个 registry，让“能力”在系统里有稳定身份，目的不在显得正式。
...
![M0 Core Kernel：真实模型接入系统边界 Mermaid 4](assets/00-09-m0-core-kernel/mermaid-04.png)
...
这属于正常的工程边界，不是“不信任模型”。模型无法调用它根本看不到的能力，系统也就少了一类无意义拒绝和提示注入风险。
...
![M0 Core Kernel：真实模型接入系统边界 Mermaid 5](assets/00-09-m0-core-kernel/mermaid-05.png)
...
Facade 的目标是让上层调用者不用直接操作 provider、event bus、state reducer、registry，不是把内部隐藏成黑盒。
...
![M0 Core Kernel：真实模型接入系统边界 Mermaid 6](assets/00-09-m0-core-kernel/mermaid-06.png)

这张图里最后一个节点很重要：M0 到这里还不执行工具；它要证明 tool intent 已经稳定进入系统边界。
...
```text
provider 输出被归一成系统事件。
tool intent 进入 pending 队列，暂不执行。
state 从 event log 来。
...
第三，provider 的职责是把外部模型响应翻译成 core `ModelEvent`；执行工具、修改 state 交给 runtime。

第四，M0 Core Kernel 的承重点是 `contracts / registry / event bus / conversation state / runtime facade`。

第五，M0 完成时，工具还没有被执行；模型事件和 tool intent 已经稳定进入系统边界，执行权仍留在 runtime 里。

写代码时，只要守住这条线：Provider 把模型能力带进系统，Core Kernel 把系统边界留在自己手里。
...
只有把这条线画清楚，后面的 Tool Runtime、Permission、Sandbox、Audit、Replay 才会顺着边界长出来，不会沦为后补补丁。
