...
U->>H: 提出任务（修登录bug）
H->>M: 发送可用工具说明
M-->>H: 返回 tool call<br/>read_file({"path": "src/auth.ts"})
H->>T: 解析并执行工具
T-->>H: 返回文件内容
H->>M: 将结果塞回上下文
M-->>H: 继续回答或发起新 tool call
```
...
```mermaid
flowchart TD
classDef think fill:#e8f5e9,stroke:#2e7d32
classDef act fill:#fff3e0,stroke:#ef6c00
classDef observe fill:#e1f5ff,stroke:#01579b
classDef endstate fill:#fce4ec,stroke:#c62828
A[用户目标：修登录跳转失败] --> B{任务完成?}
B -->|否| C[思考：先找登录代码]:::think
C --> D[行动：搜索 login/auth]:::act
D --> E[观察：找到 auth.ts、router.ts]:::observe
E --> F[思考：跳转可能在路由守卫]:::think
F --> G[行动：读取 router.ts]:::act
G --> H[观察：发现没保留 redirect]:::observe
H --> I[思考：修改 LoginPage.tsx]:::think
I --> J[行动：编辑文件]:::act
J --> K[观察：修改完成]:::observe
K --> L[行动：跑测试]:::act
L --> M[观察：测试失败]:::observe
M --> N[思考：补测试覆盖 redirect]:::think
N --> O[行动：调整实现]:::act
O --> P[观察：测试通过]:::observe
P --> B
B -->|是| Q[总结改动]:::endstate
...
A[LLM]:::model -->|Function Calling<br/>我要调用工具| B[Agent 宿主程序]:::fc
B -->|本地工具| C[本地文件系统<br/>命令执行]:::external
B -->|MCP 协议| D[MCP Server]:::mcp
D -->|标准格式| E[GitHub]:::external
D -->|标准格式| F[Postgres]:::external
D -->|标准格式| G[Slack]:::external
E -->|返回结果| D
F -->|返回结果| D
G -->|返回结果| D
D -->|返回结果| B
...
```mermaid
graph LR
classDef rigid fill:#fce4ec,stroke:#c62828
classDef flex fill:#e8f5e9,stroke:#2e7d32
classDef auto fill:#e1f5ff,stroke:#01579b
A[Workflow<br/>程序硬控流程]:::rigid --> B[Skill<br/>给方法+保留判断]:::flex
B --> C[Sub-agent<br/>放手让模型干]:::auto
```
...
classDef endstate fill:#fce4ec,stroke:#c62828
A[LLM<br/>只会生成文字] --> B[+ Tools<br/>接触外部世界]:::step
B --> C[+ Function Calling<br/>结构化请求]:::step
C --> D[+ Agent Loop<br/>多步任务循环]:::step
D --> E[+ MCP<br/>标准化接入外部能力]:::step
E --> F[+ Skill<br/>复用任务方法]:::step
F --> G[+ CLI/IDE<br/>工程现场执行]:::endstate
```
...
- [什么是 LLM：定义、能力边界与 Agent 基础](什么是-llm：定义、能力边界与-agent-基础.md)

- [Agent 概念祛魅：从 Prompt、Memory、RAG、MCP 到 Skill](agent-概念祛魅-从-prompt-memory-rag-mcp-到-skill.md)

- [Claude Code Docs Chapter 03: agent loop](wiki/AI/claude-code-docs-chapter-03-agent-loop.md)

- [Claude Code Docs Chapter 08: tools and skills](wiki/AI/claude-code-docs-chapter-08-tools-and-skills.md)

- [Claude Code Docs Chapter 09: mcp integration](wiki/AI/claude-code-docs-chapter-09-mcp-integration.md)

- [Claude Code 源码专题：扩展能力（MCP、Skills、多 Agent 与远程）](../../../wiki/AI/claude-code-源码专题-扩展能力-mcp-skills-多agent与远程.md)

- [Deep Dive Claude Code Ch12：Skill 系统](../../../wiki/AI/deep-dive-claude-code-ch12-skill-system.md)

- [Claude Code Book 11：技能系统与插件架构（教材版）](../../../wiki/AI/claude-code-book-11-技能系统与插件架构-教材版.md)

- [ReAct Agent 从 0 到 1：实现思路与步骤](wiki/AI/react-agent-从-0-到-1-实现思路与步骤.md)

- [Skill Creator](/Users/lienli/.codex/skills/.system/skill-creator/SKILL.md)
