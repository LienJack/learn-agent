...
```mermaid
flowchart TD
    A["编辑器辅助<br/>Copilot 补全代码"] --> B["对话式 LLM<br/>ChatGPT"]
    B --> C["强模型 + API<br/>GPT-4 / Function Calling"]
    C --> D["长上下文 + 工具使用<br/>Claude 2.1 / Tool Use"]
    D --> E["自治软件工程师<br/>Devin"]
    E --> F["IDE 原生 Agent<br/>Cursor Agent / Composer"]
    F --> G["GUI 操作 Agent<br/>Computer Use"]
    G --> H["标准化协议<br/>MCP"]
    H --> I["终端 Agent<br/>Claude Code / Codex CLI"]
    I --> J["个人 Agent 控制平面<br/>OpenClaw"]
    J --> K["Agent Harness 工程化<br/>运行时 / 沙箱 / 权限 / 审查"]
```
...
```mermaid
flowchart TD
    U["用户任务"] --> P["Planner<br/>任务规划"]
    P --> C["Context Manager<br/>上下文管理"]
    C --> M["LLM<br/>推理/生成"]
    M --> T["Tool Router<br/>工具选择"]
    T --> S["Sandbox<br/>执行环境"]
    S --> R["Result Reader<br/>读取结果"]
    R --> V["Verifier<br/>验证/测试"]
    V -->|失败| C
    V -->|成功| O["输出结果 / PR / 文件改动"]
```
