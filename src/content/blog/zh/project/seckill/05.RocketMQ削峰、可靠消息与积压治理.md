...
description: "围绕 Redis 预占到 PostgreSQL 落库之间的异步链路，设计 RocketMQ 普通消息、有界发送、扫描补发、Outbox 可靠发布、Inbox 幂等消费、延迟取消、积压治理、DLQ 回放和数据库背压策略。"
author: LienJack
pubDate: 2026-06-25
locale: zh
tags:
...
![第 5 章全文重点总结图：RocketMQ 削峰、可靠消息与积压治理](./assets/summary-infographics/chapter-05-summary.webp)

> 图注：本章全文重点总结图，把 RocketMQ 的削峰职责、发送结果未知、Inbox/Outbox、重试与死信队列、积压治理和延迟取消放在一张图中对照。
...
* Outbox 和 Inbox 分别解决什么问题，表结构、状态机和事务边界如何落地？
* 订单创建消息是否需要顺序？
...
![高并发秒杀订单系统：RocketMQ 削峰、可靠消息与积压治理 flow 1](./assets/05-rocketmq/mermaid-01.webp)
...
![高并发秒杀订单系统：RocketMQ 削峰、可靠消息与积压治理 flow 2](./assets/05-rocketmq/mermaid-02.webp)
...
![高并发秒杀订单系统：RocketMQ 削峰、可靠消息与积压治理 flow 3](./assets/05-rocketmq/mermaid-03.webp)
...
## 14. 消费流程、Outbox 与 Inbox 幂等事务

### 14.1 正常流程

![高并发秒杀订单系统：RocketMQ 削峰、可靠消息与积压治理 flow 4](./assets/05-rocketmq/mermaid-04.webp)

这张图里最容易被忽略的是两张“消息账本”：

| 账本 | 所在方 | 解决的问题 | 事务边界 |
| --- | --- | --- | --- |
| Outbox | 生产方数据库 | 业务事务已提交，但后续事件还没有可靠发布 | 与业务事实同一个本地事务提交 |
| Inbox | 消费方数据库 | 消息重复投递、ACK 前宕机、回放导致重复副作用 | 与消费业务效果同一个本地事务提交 |

一句话区分：

```text
Outbox 保证“该发的事件最终会被发出去”。
Inbox 保证“重复来的事件不会重复生效”。
```

Outbox 和 Inbox 不是二选一。Outbox 让消息至少一次到达下游，Inbox 承接至少一次带来的重复投递。没有 Outbox，会丢事件；没有 Inbox，会重复扣减、重复通知或重复发券。

### 14.2 Outbox 的原理

Transactional Outbox 的核心不是“多重试几次”，而是把不可原子提交的两个动作：
...
提交 PostgreSQL 订单事务
+
插入 event_outbox 待发布事件
```

订单创建成功后，系统需要发布 `ORDER_CREATED`、`ORDER_CANCEL_SCHEDULED`、库存释放或查询投影更新等后续事件。不要在订单事务里直接长时间调用 RocketMQ，因为 MQ 网络抖动会拉长数据库锁、连接和 WAL 占用。正确做法是：

```text
BEGIN
  写订单、库存流水、reservation 最终状态
  写 event_outbox
COMMIT

Outbox Publisher 异步读取 event_outbox 并发送 RocketMQ
```

这样即使服务在 `COMMIT` 后立刻宕机，`event_outbox` 记录仍然留在数据库里，发布器恢复后可以继续发送。

本章秒杀链路里有一个特殊点：Redis 预占到订单创建消息这一段还没有进入 PostgreSQL 事务，因此使用 Redis reservation 扫描补发来兜底；订单一旦进入 PostgreSQL 成为最终事实，后续跨系统事件就应该由 PostgreSQL Outbox 驱动。如果未来把 reservation 也迁移到 PostgreSQL，本章的订单创建消息也可以统一改成数据库 Outbox 或 RocketMQ 事务消息方案。

### 14.3 Outbox 表结构

`event_id` 是业务事件 ID，应该作为下游消息体里的稳定 `message_id`。RocketMQ 返回的 Broker MessageID 只用于追踪，不应作为业务幂等键。
...
CREATE TABLE event_outbox (
    ...
    status text NOT NULL CHECK (
        status IN ('PENDING', 'SENDING', 'RETRY_WAIT', 'SENT', 'DEAD')
    ),
    send_attempt integer NOT NULL DEFAULT 0,
    next_retry_at timestamptz NOT NULL DEFAULT now(),
    lease_owner text,
    lease_until timestamptz,
    rocketmq_message_id text,
    last_error text,
...
| 字段 | 含义 |
| --- | --- |
| `event_id` | 业务事件唯一 ID，下游 Inbox 使用它去重 |
| `aggregate_type` / `aggregate_id` | 事件所属聚合，例如 `order/ord_01...` |
| `topic` / `tag` | RocketMQ 目标 Topic 和 Tag |
| `message_key` | RocketMQ Key，通常用 `order_id` 或 `reservation_id` 便于检索 |
| `message_group` | 需要 FIFO 时才填写，例如订单状态事件使用 `order_id` |
| `payload` | 完整事件体，必须包含 `schema_version` 和稳定业务 ID |
| `status` | 发布状态，不等于业务订单状态 |
| `send_attempt` / `next_retry_at` | 退避重试控制 |
| `lease_owner` / `lease_until` | 多发布器并发领取时避免同一时刻重复发送 |
| `rocketmq_message_id` | Broker 返回的消息 ID，仅用于排查 |
...
如果订单创建事务回滚，Outbox 也一起回滚，下游不会收到一个并不存在的订单事件。

### 14.4 Outbox Publisher 实现

发布器是一个后台 Worker 集群。它不参与订单创建事务，只负责把 `event_outbox` 里的待发布事件至少一次发送到 RocketMQ。
...
1. 数据库事务只负责“领取任务”，提交后再调用 RocketMQ，不要持有行锁等待网络发送。
2. `lease_until` 到期后仍是 `SENDING` 的记录可以被重新置为 `RETRY_WAIT`，防止发布器宕机造成永久卡住。

发送结果处理：

| 发送结果 | Outbox 状态 | 说明 |
| --- | --- | --- |
| 明确成功 | `SENT` | 记录 `rocketmq_message_id`、`sent_at` |
| 可重试失败 | `RETRY_WAIT` | 写 `last_error`，按指数退避设置 `next_retry_at` |
| 结果未知 | `RETRY_WAIT` | 不能删除记录，稍后用同一 `event_id` 再发 |
| 确定性错误 | `DEAD` | Topic、ACL、schema 等配置错误，告警并人工修复 |

最关键的故障窗口是：

```text
Outbox Publisher 已经把消息发到 RocketMQ
→ 还没来得及把 event_outbox 标记为 SENT
→ 进程宕机
```

恢复后发布器会再次发送同一个 `event_id`。这就是为什么 Outbox 只能提供至少一次发布，不能单独承诺下游业务效果 Exactly Once。下游必须用 Inbox、唯一约束或状态机接住重复事件。

### 14.5 Inbox 的原理

Inbox 是消费方自己的“已处理消息账本”。它不依赖 RocketMQ 的 Broker MessageID，而是依赖生产方生成的稳定业务 `message_id`。唯一键必须包含 `consumer_group`：

```text
UNIQUE (consumer_group, message_id)
```

原因是同一条 `ORDER_CREATED` 事件可能同时被多个下游处理：

```text
CG_ORDER_QUERY_PROJECTION
CG_COUPON_GRANT
CG_NOTIFICATION
CG_AUDIT
```

这些 Consumer Group 的副作用不同，不能因为订单消费者处理过，就阻止通知消费者处理。每个 Consumer Group 都要有自己的 Inbox 记录。

Inbox 的正确性来自三个动作处于同一事务：
...
执行业务写入
+
更新 consumer_inbox 终态
```

正常实现中，`PROCESSING` 不应该单独提交成长期可见状态。它只是当前事务内部的中间状态；事务成功时会一起提交为 `SUCCEEDED` 或 `REJECTED`，事务失败时会一起回滚。若线上出现长期 `PROCESSING`，通常表示代码在事务边界上出现了漏洞，需要对账修复，而不是盲目 ACK。

### 14.6 为什么 Inbox 必须和业务写入同事务
...
CREATE INDEX idx_consumer_inbox_order
    ON consumer_inbox (order_id, created_at);
```
...
affected rows = 0：
本 Consumer Group 已经处理过该 message_id，或正在等待另一个未提交事务结束后确认冲突。
```

PostgreSQL 唯一约束会让并发重复插入收敛：如果两个消费者同时处理同一条消息，一个事务成功提交，另一个事务的 `ON CONFLICT DO NOTHING` 最终得到 `affected rows = 0`。如果第一个事务回滚，第二个事务可以继续插入并处理。
...
func (h *OrderHandler) processTransaction(
	...
	/*
		...
		7. 写 event_outbox，驱动后续查询投影、延迟取消或补偿事件。
	...
	*/
	...
	if err := appendOrderOutboxEvents(ctx, tx, event); err != nil {
		return ProcessResult{}, err
	}
...
![高并发秒杀订单系统：RocketMQ 削峰、可靠消息与积压治理 flow 5](./assets/05-rocketmq/mermaid-05.webp)
...
![高并发秒杀订单系统：RocketMQ 削峰、可靠消息与积压治理 flow 6](./assets/05-rocketmq/mermaid-06.webp)
...
![高并发秒杀订单系统：RocketMQ 削峰、可靠消息与积压治理 flow 7](./assets/05-rocketmq/mermaid-07.webp)
...
### 26.4 PostgreSQL Outbox

**优化点：** 订单事务只写本地 Outbox，后续 RocketMQ 事件由后台发布器异步发送。

**要解决的问题：** 订单已创建，但延迟取消、查询投影、库存补偿等后续事件可能因进程宕机或 MQ 超时丢失。

**未经优化时会发生什么：** 事务提交后服务崩溃，下游永远不知道订单已创建；或者在事务内等待 MQ，导致数据库锁和连接被外部网络拖慢。
...
**底层原理：** 把“数据库业务表 + MQ”双写变成“数据库业务表 + event_outbox”单库事务，再用可重放账本补齐 MQ 发布。

**预计收益：** 业务事实只要提交，后续事件就不会无声丢失。

**代价和副作用：** 发布可能重复，下游必须使用 Inbox 或业务幂等；Outbox 表需要分区、归档和积压告警。

**适用边界：** 业务事实已经落在 PostgreSQL，且后续事件允许最终一致。

**不适用场景：** 必须同步返回下游处理结果的强交互流程，此时应重新评估业务边界或使用工作流编排。
...
outbox_publish_total{outcome}
outbox_dead_count
```

**验证方法：** 在 Publisher 发送成功后、标记 `SENT` 前强杀进程，验证重启后会重复发送，但下游 Inbox 不产生重复业务效果。
...
| Outbox 已发送但未标记 SENT | 下游可能收到重复事件        | `outbox_retry`、Inbox 冲突   | 同 event_id 重发，下游幂等 | 长期积压时检查发布器        |
...
```text
outbox_ready_count{topic}
outbox_oldest_age_seconds{topic}
outbox_publish_total{topic,outcome}
outbox_publish_latency_seconds{topic}
outbox_dead_count{topic}
outbox_lease_expired_total{topic}
```
...
### 9. Outbox 和 Inbox 分别解决什么问题？

Outbox 解决生产侧可靠发布：业务事务已经提交，后续事件必须最终发出去。它把“业务表 + MQ”的跨系统双写改成“业务表 + event_outbox”的同库事务，再由发布器至少一次发送。

Inbox 解决消费侧幂等：同一事件可能因为发送重试、ACK 前宕机、DLQ 回放或 Outbox 重发而多次到达。消费者先在同一事务里插入 `(consumer_group, message_id)`，再做业务写入，最后提交后 ACK。
...
6. **消费者在 PostgreSQL 本地事务中完成 Inbox、最终库存、订单、流水和 Outbox，Outbox 再由发布器至少一次发送后续事件。**
