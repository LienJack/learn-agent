...
![第 3 章全文重点总结图：接入层流量治理与系统保护](./assets/summary-infographics/chapter-03-summary.webp)

> 图注：本章全文重点总结图，用多级漏斗展示 CDN/WAF、资格票、网关限流、Go 本地限流和 Redis 全局准入如何把无效流量挡在低成本位置。
...
![高并发秒杀订单系统：接入层流量治理与系统保护 flow 1](./assets/03-leik5j/mermaid-01.webp)
...
![高并发秒杀订单系统：接入层流量治理与系统保护 flow 2](./assets/03-leik5j/mermaid-02.webp)
...
| GCRA 虚拟漏桶 | 保存理论到达时间 `tat`，判断请求是否来得太早 | 状态极少，天然能计算 `Retry-After` | 参数不如令牌桶直观，不是真实排队 | 登录、短信、第三方 API、来源限流 |
...
| --- | --- |
| `tokens` | 当前剩余令牌数 |
| `last_refill_time` | 上次计算补充令牌的时间 |

每个请求到来时：

1. 根据当前时间和 `last_refill_time` 计算过去时间内应补充的令牌。
2. 使用 `min(capacity, tokens + elapsed * rate)` 限制桶容量。
3. 如果 `tokens >= cost`，扣减 `cost` 并放行。
4. 否则拒绝，并根据缺口计算 `Retry-After`。
...
含义是长期平均最多 100 QPS，但系统空闲后可以积累最多 300 个令牌，因此能吸收最多 300 个短突发请求。这个特性适合秒杀入口，因为用户连点、移动网络抖动后的集中重试和前端短重试不一定都是攻击。

落地时需要注意四点：

* 多实例全局限流应使用 Redis 服务器时间，而不是让每个网关传本机时间。
* Redis Lua 里可以用整数缩放，例如 `1` 个令牌表示为 `1000` 个毫令牌，减少浮点累计误差。
* 新 Key 默认满桶会改善正常用户体验，但攻击者轮换 Key 可能吃到重复 burst，所以必须组合用户、设备、IP、Key 数量上限和溢出桶。
* TTL 至少覆盖两次从空桶补满的时间，避免活跃限流状态过早消失。
...
GCRA 全称是 Generic Cell Rate Algorithm。放到网关限流里，可以把它理解成**不真实排队的漏桶**：它不保存队列，也不保存当前令牌数，只保存一个理论到达时间：

```text
tat = theoretical arrival time
```

如果限流速率是 `rate`，则每个请求的理论间隔为：

\[
interval = \frac{1}{rate}
\]

突发容忍度为：

\[
tolerance = (burst - 1) \times interval
\]

请求到达时，先计算最早允许时间：

\[
allow\_at = tat - tolerance
\]
...
```text
interval = 200ms
tolerance = (3 - 1) * 200ms = 400ms
```

如果 0ms 同时来了 4 个请求，前三个会把 `tat` 依次推进到 `200ms`、`400ms`、`600ms`。第 4 个请求看到：

```text
allow_at = 600ms - 400ms = 200ms
now = 0ms
```

因此被拒绝，`Retry-After` 约为 `200ms`。

普通 API 网关更常用令牌桶，因为它的 `rate + capacity` 参数直观，允许合理突发。登录、短信、验证码、第三方 API 和来源 IP 这类“不希望请求挤成一团”的接口，更适合使用 GCRA 或滑动窗口。秒杀提交接口通常仍以令牌桶做入口保护，用边缘排队和 MQ 削峰来处理后续节奏。
...
![高并发秒杀订单系统：接入层流量治理与系统保护 flow 3](./assets/03-leik5j/mermaid-03.webp)
...
![高并发秒杀订单系统：接入层流量治理与系统保护 flow 4](./assets/03-leik5j/mermaid-04.webp)
...
![高并发秒杀订单系统：接入层流量治理与系统保护 flow 5](./assets/03-leik5j/mermaid-05.webp)
...
## 9.3 Redis Lua 全局 GCRA

GCRA 适合需要更平滑节奏的维度，例如登录、短信、验证码、第三方 API 或来源 IP。它只需要保存一个 `tat`，可以直接使用 Redis String。
...
```text
ARGV[1]：interval_us，每个单位请求之间的理论间隔，单位微秒
ARGV[2]：burst，允许的突发单位数
ARGV[3]：cost，本次请求消耗的单位数，普通请求为 1
ARGV[4]：ttl_ms，Key TTL
```
...
```text
[code, retry_after_ms, tat_us, redis_now_us]

code = 1   允许
code = 0   限流
code = -1  参数错误
code = -2  cost 大于 burst，单次请求永远无法通过
```
...
local base_tat = math.max(now_us, tat)
local candidate_tat = base_tat + cost * interval_us
local burst_window_us = burst * interval_us

-- 等价于 cost=1 时的：
-- allow_at = tat - (burst - 1) * interval_us
-- now_us >= allow_at
if candidate_tat > now_us + burst_window_us then
    local retry_after_us = candidate_tat -
        (now_us + burst_window_us)
    local retry_after_ms = math.floor(
        (retry_after_us + 999) / 1000
    )
...
interval_us = 1_000_000 / 10 = 100_000
ttl_ms = interval_ms * burst * 2 = 100 * 5 * 2 = 1000
```

如果只处理普通请求，`cost` 固定传 `1` 即可。若某些请求成本更高，例如一次批量查询相当于 5 个普通请求，则可以传 `cost = 5`，脚本会把 `tat` 一次性向后推进 `5 * interval_us`。
...
## 16.5 令牌桶、漏桶和 GCRA 有什么区别？

令牌桶限制长期平均速率，但允许不超过桶容量的突发，适合用户重试和入口流量。

漏桶以更稳定的速率输出，更适合保护需要平滑负载的下游。如果漏桶通过排队实现，可能增加同步请求延迟。因此秒杀提交更适合“令牌桶立即拒绝”，而不是“漏桶长时间等待”。

GCRA 是一种虚拟漏桶。它不维护真实队列，只维护 `tat`。请求到达时，如果 `now < tat - tolerance`，说明来得太早，直接返回 `429` 和 `Retry-After`；否则放行，并更新 `tat = max(now, tat) + interval`。

面试中可以这样区分：

| 算法 | 核心问题 | 状态 | 更适合 |
| --- | --- | --- | --- |
| 令牌桶 | 现在有没有令牌 | `tokens + last_refill_time` | 秒杀入口、通用 API、允许短突发的用户请求 |
| 漏桶 | 如何以固定速率流出 | 队列或出水时间 | 异步任务、下游平滑 |
| GCRA | 你是不是来得太早 | `tat` | 登录、短信、验证码、第三方 API、来源限流 |
...
5. 令牌桶适合入口突发保护，GCRA 适合更平滑的行为频率控制。
6. 本地限流提供低延迟实例保护，全局限流提供集群统一预算。
7. 热点 SKU 不应让每个请求访问同一个全局限流 Key，应优先使用本地预算和令牌租约。
8. 秒杀同步请求应立即接受或拒绝，不应在 Go 服务中长时间等待。
9. 本地售罄标记只能作为性能优化，必须使用版本、epoch、TTL 和重新开放机制。
10. 静态限流不能处理依赖退化，还必须根据健康指标进行自适应负载丢弃。
11. 客户端重试必须复用同一 `request_id`，采用有界重试、指数退避和随机抖动。
12. 新抢购在关键准入组件不可用时默认 Fail Closed。
13. 所有准入决策都不能替代 Redis 和 PostgreSQL 对库存与一人一单的最终校验。
