...
description: "围绕积分报价、额度预占、结算、释放、退款、成本归集、供应商账单、RocketMQ 重复投递、幂等账本、余额快照、差异对账、预算控制和成本感知路由，设计 AI 视频平台的计费、成本与对账系统。"
author: LienJack
pubDate: 2026-06-25
locale: zh
tags:
...
![第 20 章全文重点总结图：计费、成本与对账](./assets/summary-infographics/chapter-20-summary.webp)

> 图注：本章全文重点总结图，围绕报价、预占、结算、释放、退款、不可变账本、供应商成本、成本感知路由、SUBMIT_UNKNOWN 和每日对账闭环展开。
...
→ 成功后结算积分并释放差额
→ 失败或取消时按策略释放、退款或补偿
→ 每日用内部账本、任务状态和供应商账单做对账
```

这个回答的重点不是“任务结束时减余额”，而是把积分系统拆成三条事实链：

1. **用户积分链**：报价、预占、结算、释放、退款和补偿。
2. **任务执行链**：task、attempt、供应商任务、回调、轮询和最终状态。
3. **成本对账链**：供应商实际收费、平台媒体成本、内部成本事件和外部账单。
...
面试时如果时间很短，就围绕“预占防止欠费、唯一业务键防止多扣、状态机处理竞态、对账补齐跨系统不确定性”展开。
...
#### 积分扣费主链路：一笔任务如何从预占走到结算

以“用户生成一条 10 秒 1080p 视频，报价 120 credits，最终按 95 credits 结算”为例，落地流程如下：

```text
1. Client 提交生成请求，携带 Idempotency-Key。
2. API 根据模型、时长、分辨率、优先级做服务端报价，得到 max_reserve_amount=120。
3. API 开启 PostgreSQL 事务。
4. 写入或读取 idempotency_request，保证同一请求只创建一个 task。
5. 锁定 wallet_account，检查 available_amount >= 120。
6. 追加 reserve:generation:{task_id}:v1 账本事务。
7. ledger_entry 写入 AVAILABLE -120、RESERVED +120。
8. wallet_account 快照同步更新为 available 减 120、reserved 加 120。
9. 创建 credit_reservation、generation_task、generation_charge 和 outbox_event。
10. 事务提交后由 Outbox Relay 投递 RocketMQ，调度供应商执行。
11. 任务成功后 Settlement Consumer 锁定 reservation。
12. 追加 settle:generation:{task_id}:v1，RESERVED -95、CONSUMED +95。
13. 同一事务追加 release:generation:{task_id}:final-delta，RESERVED -25、AVAILABLE +25。
14. 更新 generation_charge.final_charge_amount=95，任务进入已结算。
```

用户在界面上看到的变化是：

```text
提交前：可用 500，冻结 0，已消费 0
预占后：可用 380，冻结 120，已消费 0
结算后：可用 405，冻结 0，已消费 95
```

这比“完成后直接扣 95”多了一步冻结，但它解决了两个关键问题：任务进入供应商前已经确认用户有额度；任务执行期间可用余额不会被其他任务花掉。

#### 扣费接口返回什么

创建任务接口不要只返回“扣费成功”。更合理的是返回一份可解释的扣费快照：

```json
{
  "task_id": "task_123",
  "reservation_id": "resv_123",
  "price_version_id": "price_2026_06",
  "quoted_amount": 120,
  "reserved_amount": 120,
  "wallet_after": {
...
这样客服、用户账单页和对账任务都能解释“为什么先冻结 120，最后只消费 95”。
...
积分扣费系统要按“入口防重、状态防重、账本防重、消息可重放”来设计。
...
用户双击提交、前端超时重试、网关重试，都必须命中同一条幂等记录。若 `request_hash` 不同，说明同一个 key 被用于不同请求，应返回冲突，而不是创建新任务。

第二道是任务和 reservation 的唯一关系：

```text
unique (generation_task.client_request_id)
unique (credit_reservation.task_id)
unique (generation_charge.task_id)
```

这保证一个生成任务不会有两条主 reservation，也不会出现两套收费快照。
...
真正防多扣的是 `ledger_transaction(wallet_id, business_key)` 唯一约束。消费者收到重复结算事件时，如果 `settle:generation:{task_id}:v1` 已经存在，就直接返回已处理，不再更新余额。
...
where reservation_id = $2
  and state in ('ACTIVE', 'PARTIALLY_SETTLED')
  and settled_amount + released_amount + $1 <= reserved_amount
  and version = $3;
```

这能挡住取消、完成、退款、重复结算之间的并发竞争。账本唯一键保证“同一业务动作不重复”，状态条件保证“当前状态允许这个动作”。

#### 不能把这些东西当成最终防线

下面这些能力有用，但不能单独承担“防多扣”：

| 能力 | 可以做什么 | 不能做什么 |
|---|---|---|
| Redis `SETNX` | 降低短时间重复提交压力 | 不能保证崩溃、过期、主从切换后的账务唯一 |
| RocketMQ 消息 ID | 辅助排查重复消息 | 不能代表业务动作唯一 |
| 前端按钮置灰 | 改善体验 | 不能防刷新、重放、脚本或网关重试 |
| 分布式锁 | 减少并发冲突 | 不能替代数据库唯一约束和事务 |
| 回调 event id | 防供应商回调重复 | 不能防 Polling 与 Callback 产生同一个业务终态 |

所以最终原则是：**所有会改变积分余额的动作，都必须落到 PostgreSQL 本地事务里，并由业务唯一键约束。**
...
```text
InvoiceFetchJob
  拉取供应商账单 / usage export
  保存原始文件到对象存储
  记录 source_object_key 和 checksum

InvoiceNormalizeJob
  把不同供应商字段转成 invoice_line
  统一 provider_account、provider_task_id、charge_id、amount、currency、occurred_at

ReconciliationMatchJob
  用 charge_id / provider_task_id / external_idempotency_key 精确匹配内部 attempt
  生成 matched 或 discrepancy

ReconciliationResolveJob
  对低风险确定性差异生成自动修复候选
  对高风险差异生成工单，等待人工复核
```
...
如果没有 `external_charge_id`，至少要用 `raw_line_hash` 防止同一账单文件重复导入；但这只能防导入重复，不能证明业务匹配唯一。
...
| 差异 | 示例 | 处理方式 |
|---|---|---|
| 内部有结算，供应商无账单 | 本地认为生成成功并向用户收费，但供应商账单没有费用 | 标记为 `MISSING_EXTERNAL`，等待跨日补账；若确认供应商免费，不自动改用户账 |
| 供应商有账单，内部无 attempt | 提交超时后本地没有记录 provider_task_id，但供应商收费 | 标记为 `MISSING_INTERNAL`，补录成本事件，关联到 `SUBMIT_UNKNOWN` 或人工确认任务 |
| 内部金额与外部金额不同 | 预估 0.8 美元，账单 1.1 美元 | 追加 provider cost adjustment，更新成本报表，不直接改用户扣费 |
| 同一个外部 charge 出现两次 | 账单文件重复或供应商重复收费 | 去重导入；若供应商真实重复收费，记录 dispute 并等待 credit note |
| 用户被多扣 | 同一 task 出现两笔 SETTLE | 理论上应被唯一键挡住；若历史 bug 产生，追加 REFUND 或 COMPENSATE，不删除原流水 |
| 用户被少扣或漏扣 | 任务成功但没有 settle | 根据收费政策和时效生成补扣候选；高风险场景先人工审核 |

对账修复要遵循两个边界：

1. **供应商成本差异不等于用户价格差异。** 供应商多收通常先影响平台成本，不应该自动二次扣用户。
2. **历史账本不可覆盖。** 多扣退款、少扣补扣、成本补录都通过新的 ledger 或 cost event 表达。

#### 对账报告应该给谁看

日对账报告至少面向三类角色：

- **技术值班**：看 `SUBMIT_UNKNOWN`、未匹配账单、重复 attempt、成本事件积压。
- **运营/客服**：看可能影响用户余额的多扣、漏扣、退款失败和补偿工单。
- **财务/业务负责人**：看供应商账单总额、平台收入、毛利、信用额度和预算执行。

报告里不要只给一个“对账成功率”。更有价值的指标是：

```text
未匹配外部账单金额
未匹配内部 attempt 数
金额差异总额和差异率
SUBMIT_UNKNOWN 仍未收敛数量
重复生成成本占比
用户多扣 / 少扣风险金额
自动修复数量与人工待处理数量
```
...
## 20.15 面试官可能追问的 12 个问题与资深回答

### 问题 1：如果让你设计一个积分扣费系统，你会怎么讲整体流程？

**资深回答：**

我会先把它设计成“报价、预占、执行、结算、释放、退款、对账”的闭环，而不是任务完成后直接扣余额。用户提交 AI 视频任务时，服务端根据模型、时长、分辨率和价格版本计算报价，然后在 PostgreSQL 本地事务中锁定钱包，追加 RESERVE 账本，更新余额快照，创建 reservation、task 和 Outbox。事务提交后任务才进入 RocketMQ 和供应商调度。任务成功后，结算消费者根据稳定业务键追加 SETTLE；如果实际收费低于预占，同事务 RELEASE 差额。失败或取消时按供应商阶段释放、结算取消费或退款。每天再用内部账本、任务状态、provider attempt 和供应商账单做对账，差异通过补偿流水或成本调整闭环。

### 问题 2：为什么不在任务完成后直接扣款，为什么一定要预占？
...
### 问题 4：用户连续点击两次生成按钮，会不会创建两笔预占？

**资深回答：**

不会。客户端每次提交携带 `Idempotency-Key`，服务端在 `idempotency_request(tenant_id, idempotency_key)` 上做唯一约束，并保存 request hash。第一次请求创建 task、reservation 和 RESERVE 流水；第二次如果参数完全一致，直接返回第一次的 task 和扣费快照；如果同一个 key 对应不同参数，返回幂等冲突。即使两次请求同时进入，数据库唯一约束也只有一个事务能成功，另一个事务回滚后读取已有结果。前端置灰只能提升体验，真正防重必须在数据库业务键上。

### 问题 5：供应商已经受理，但本地提交超时，怎么处理？
...
### 问题 8：不可变账本和余额快照会不会重复保存事实？怎样保证一致？
...
### 问题 12：Billing PostgreSQL 故障时系统怎样降级？
