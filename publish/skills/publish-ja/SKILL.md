...
5. 展示 Zenn 仓库状态、生成文件、图片数量、`published` 状态、Qiita item/visibility、Qiita warnings、Qiita 图片 URL 检查结果和拟提交范围。
6. 只有用户确认后，运行 `publish/bin/publish-ja --yes <article.md> [...]` 执行 Zenn commit/push 与 Qiita 写入。
7. 如果用户希望生成公开文章，确认前明确说明会使用 `--published`：Zenn 会公开，Qiita 也会 public；不带 `--published` 时，Zenn draft 且 Qiita private。
